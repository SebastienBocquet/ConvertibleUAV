...
namespace 
{
    ...
    TEST_F(TxLinearControl, ComputeTxLinearControl)
    {
        ...
        int16_t output_control = compute_tx_linear_control(tx_order, output_min, output_max, reverse_control);
        ASSERT_EQ(output_control, 0);

        tx_order = 4000;
        output_control = compute_tx_linear_control(tx_order, output_min, output_max, reverse_control);
