...
void initMPUSPI_master16(uint16_t priPre, uint16_t secPre) {
  /*
          uint16_t SPICON1Value;      // holds the information about SPI
  configuration
          uint16_t SPICON2Value;
          uint16_t SPISTATValue;      // holds the information about SPI
  Enable/Disable
  ...
  BUF_INT_SEL_5;
          // BUF_INT_SEL_5 == Interrupt when the last bit is shifted out of
  SPIxSR, and the transmit is complete
  #else
          SPICON1Value =
              ENABLE_SDO_PIN & SPI_MODE16_ON & ENABLE_SCK_PIN &
  ...
  #ifdef __PIC32MX__
  //	 * Example:
  OpenSPI1(SPI_MODE32_ON|SPI_SMP_ON|MASTER_ENABLE_ON|SEC_PRESCAL_1_1|PRI_PRESCAL_1_1,
  SPI_ENABLE);
...
void writeMPUSPIreg16(uint16_t addr, uint16_t data) {
  /*
          ...
          // this delay is necessary; it appears that SS must be deasserted for
  one or
...
void __attribute__((__interrupt__, __no_auto_psv__)) SPIInterrupt(void) {
  ...
  _SPIIF = 0;  // clear interrupt flag as soon as possible so as to not miss any
               // interrupts
...
void __attribute__((__interrupt__, __no_auto_psv__)) SPIInterrupt(void) {
  /*
          ...
          _SPIIF = 0;                 // clear interrupt flag as soon as
  possible so as to not miss any interrupts
          indicate_loading_inter;
          interrupt_save_set_corcon;
  #if 1
          if (SPI_i == 0) {
                  spibuf = SPIBUF;                               // could move
  this to before the conditional
                  SPIBUF = 0x0000;
                  SPI_high = 0xFF & spibuf;                      // could move
  this to after the conditional
                  SPI_i = 1;
          } else if (SPI_i < SPI_n) {
                  spibuf = SPIBUF;                               // could move
  this to before the conditional
                  SPIBUF = 0x0000;
                  SPI_low = spibuf >> 8;                         // could move
  this to before the conditional
                  *(SPI_data + SPI_j) = SPI_high << 8 | SPI_low; // could move
  this to before the conditional
                  SPI_high = 0xFF & spibuf;                      // could move
  this to after the conditional
                  SPI_i++;
