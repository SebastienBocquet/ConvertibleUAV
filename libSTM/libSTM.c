...
uint8_t rc_signal_strength;  // rc_signal_strength is 0-100 as percent of full
                             // signal
...
uint16_t get_reset_flags(void) { return mp_rcon; }
...
void I2C_doneReadMagData(void) {
  ...
  if (magMessage == 7) {
    udb_magFieldBody[0] = MAG_X_SIGN((__builtin_mulsu((magFieldRaw[MAG_X_AXIS]),
                                                      magGain[MAG_X_AXIS])) >>
                                     14) -
                          (udb_magOffset[0] >> 1);
    udb_magFieldBody[1] = MAG_Y_SIGN((__builtin_mulsu((magFieldRaw[MAG_Y_AXIS]),
                                                      magGain[MAG_Y_AXIS])) >>
                                     14) -
                          (udb_magOffset[1] >> 1);
    udb_magFieldBody[2] = MAG_Z_SIGN((__builtin_mulsu((magFieldRaw[MAG_Z_AXIS]),
                                                      magGain[MAG_Z_AXIS])) >>
                                     14) -
                          (udb_magOffset[2] >> 1);
...
uint16_t register_event_p(void (*event_callback)(void),
...
// void Reset_Handler(void) {} // this must be loosely defined in the startup
// code and the default seems to call main()
// void mcu_init(void) {} // now provided by main() in the STMCubeMX generated
// code (redef'd to mcu_init()
