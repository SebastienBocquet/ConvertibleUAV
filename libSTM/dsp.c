...
static fractional fl2fr(float fl) { return (int16_t)(fl * 32768.0); }

static float fr2fl(fractional fr) { return (fr / 32768.0); }
...
fractional* MatrixMultiply(/* Matrix multiplication */
                           ...
                           int16_t numCols1Rows2, /* number columns in srcM1,
                                                     same as */
  ...
  for (i = 0; i < numRows1; i++) {
    for (j = 0; j < numCols2; j++) {
      ...
      for (k = 0; k < numCols1Rows2; k++) {
        dstM[MatrixIndex(j, i, numCols2)] +=
            fl2fr(fr2fl(srcM1[MatrixIndex(k, i, numCols1Rows2)]) *
                  fr2fl(srcM2[MatrixIndex(j, k, numCols2)]));
      }
    }
  }
  return dstM;
...
fractional*
VectorMultiply(/* Vector elem-to-elem multiply */
               /* dstV[elem] =                 */
               /*    = srcV1[elem] * srcV2[elem] */
               /* (in place capable) */
               /* (with itself capable) */
