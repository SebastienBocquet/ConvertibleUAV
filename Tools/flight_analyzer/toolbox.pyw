...
def exp_filter_discrete(signal, invdeltafilter, heartbeat_hz):
    ...
    for i in range(1, len(signal)):
        filtered_signal[i] = np.floor((np.round(invdeltafilter * signal[i]) + \
            np.round((heartbeat_hz - invdeltafilter) * filtered_signal[i - 1])) / heartbeat_hz)
...
def exp_filter(signal, invdeltafilter, heartbeat_hz):
    ...
    for i in range(1, len(signal)):
        filtered_signal[i] = (invdeltafilter * signal[i] + \
            (heartbeat_hz - invdeltafilter) * filtered_signal[i - 1]) / heartbeat_hz
