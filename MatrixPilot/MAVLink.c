...
// Mavlink Aviation  X Axis is the UDB Aviation Y axis which is the fuselage
// axis.
// Mavlink Avitation Y axis is out of the right wing, and so is the negative of
// the UDB X Axis
// Mavlink Aviation  Z axis is downward from the plane, ans so is the same as
// UDB Z axis.
// Mavlink Yaw is positive to the right (same as UDB)
// Pitch is positive when the front of the plane pitches up from horizontal
// (opposite of UDB)
...
//    MAV_DATA_STREAM_EXTRA2 = Scaled position sensor messages (ALTITUDES /
// AIRSPEEDS)
...
// Setting MAVLINK_TEST_ENCODE_DECODE to 1, will replace the normal code that
// sends MAVLink messages with
// as test suite.  The inserted code will self-test every message type to encode
// packets, de-code packets,
// and it will then check that the results match. The code reports a pass rate
// and fail rate
// out of the serial port (sent as normal ascii). There should never be any
// fails. The code
// runs purely within the UAV DevBoard so this purely tests software, not the
// communication links.
// Normal default is to set MAVLINK_TEST_ENCODE_DECODE to 0

// This testing section of code only compiles if you set the C-Compiler to use
// the "Large memory code model"
// In MPLAB IDE, select "Project / Build Options / Project", then select Tab
// MPLAB C30. Then select the
// drop down menu called "Categores" and select "Memory Model". Tick "Large Code
// Model" instead of
...
// The following Macro enables MAVLink packets to be sent in one call to the
// serial driver
...
int16_t mavlink_serial_send(mavlink_channel_t chan, uint8_t buf[],
                            uint16_t len);
...
#define BYTE_CIR_16_TO_RAD \
  ((2.0 * 3.14159265) / 65536.0)  // Conveert 16 bit byte circular to radians
...
uint64_t usec =
    0;  // A measure of time in microseconds (should be from Unix Epoch).
uint32_t msec =
    0;  // A measure of time in microseconds (should be from Unix Epoch).
...
// Following are required for saving state of PWM variables for SERIAL_UDB_EXTRA
// compatibility
#define MAVLINK_SUE_CHANNEL_MAX_SIZE \
  10  //  MatrixPilot.xml MAVLink has fixed SUE protocol for 10 channels
int16_t pwIn_save[MAVLINK_SUE_CHANNEL_MAX_SIZE + 1];
int16_t pwOut_save[MAVLINK_SUE_CHANNEL_MAX_SIZE + 1];

int16_t mavlink_sue_telemetry_counter =
    8;  // Countdown counter, for use with SERIAL_UDB_EXTRA compatibility
...
void init_mavlink(void) {
  mavlink_process_message_handle =
      register_event_p(&handleMessage, EVENT_PRIORITY_MEDIUM);
  mavlink_system.sysid =
      MAVLINK_SYSID;  // System ID, 1-255, ID of your Plane for GCS
  mavlink_system.compid =
      1;  // Component/Subsystem ID,  (1-255) MatrixPilot on UDB is component 1.
...
int16_t udb_serial_callback_get_byte_to_send(void) {
  if (sb_index < end_index &&
      sb_index < SERIAL_BUFFER_SIZE)  // ensure never end up racing thru memory.
  {
...
int16_t mavlink_serial_send(mavlink_channel_t UNUSED(chan), uint8_t buf[],
                            uint16_t len)
...
#if (MAVLINK_TEST_ENCODE_DECODE == 1)
// add printf library when running tests to output ascii messages of test
// results
void serial_output(char *format, ...) {
  ...
  wrote = vsnprintf((char *)(&mavlink_test_message_buffer[0]),
                    (size_t)remaining, format, arglist);
  if (wrote > 0) {
    mavlink_serial_send(MAVLINK_COMM_0, &mavlink_test_message_buffer[0],
                        (uint16_t)wrote);
  }
}
#endif
...
void mp_mavlink_transmit(uint8_t ch)
    ...
    // This will then be checked against the original message and results
    // recorded
...
#else
void mp_mavlink_transmit(uint8_t ch)
    // routine to send a single character used by MAVlink standard include
    // routines.
    // We forward to multi-byte sending routine so that firmware can interleave
    // ascii debug messages with MAVLink binary messages without them
...
////////////////////////////////////////////////////////////////////////////////////////
//
// MAIN MATRIXPILOT MAVLINK CODE FOR RECEIVING COMMANDS FROM THE GROUND CONTROL
// STATION
//
...
void udb_serial_callback_received_byte(uint8_t rxchar) {
  if (mavlink_parse_char(0, rxchar, &msg[mavlink_message_index],
                         &r_mavlink_status)) {
    // Check that handling of previous message has completed before calling
    // again
    if (handling_of_message_completed == true) {
      // Switch between incoming message buffers
      if (mavlink_message_index == 0)
        mavlink_message_index = 1;
      else
...
// All paramaters are sent as type (mavlink_param_union_t) between Ground
// Control Station and MatrixPilot.
// So paramaters have to be converted between type (mavlink_param_union_t) and
// their normal representation.
// An explanation of the MAVLink protocol for changing paramaters can be found
// at:
...
boolean mavlink_parameter_out_of_bounds(mavlink_param_union_t parm, int16_t i) {
  switch (mavlink_parameter_parsers[mavlink_parameters_list[i].udb_param_type]
              .mavlink_type) {
...
void mavlink_send_param_maxstack(int16_t i) {
  mavlink_msg_param_value_send(MAVLINK_COMM_0, mavlink_parameters_list[i].name,
                               (4096 - maxstack), MAVLINK_TYPE_FLOAT,
                               count_of_parameters_list, i);
  // mavlink_msg_param_value_send(mavlink_channel_t chan, const char *param_id,
  // float param_value, uint8_t param_type, uint16_t param_count, uint16_t
  // param_index)
}
...
void mavlink_set_maxstack(float setting, int16_t i) {
  ...
  if ((mavlink_parameters_list[i].readonly == true) ||
      (mavlink_parameter_out_of_bounds(param, i) == true))
    return;
...
void mavlink_send_param_gyroscale_Q14(int16_t i) {
  mavlink_msg_param_value_send(
      MAVLINK_COMM_0, mavlink_parameters_list[i].name,
      (float)(*((int16_t *)mavlink_parameters_list[i].pparam) /
              (SCALEGYRO * 16384.0)),
      MAVLINK_TYPE_FLOAT, count_of_parameters_list,
      i);  // 16384.0 is RMAX defined as a float.
}

void mavlink_set_param_gyroscale_Q14(mavlink_param_union_t setting, int16_t i) {
  if (setting.type != MAVLINK_TYPE_FLOAT) return;

  *((int16_t *)mavlink_parameters_list[i].pparam) =
      (int16_t)(setting.param_float * (SCALEGYRO * 16384.0));
}

void mavlink_send_param_Q14(int16_t i) {
#if (QGROUNDCTONROL_PID_COMPATIBILITY == \
     1)  // see mavlink_options.h for details
  mavlink_msg_param_value_send(
      MAVLINK_COMM_0, mavlink_parameters_list[i].name,
      (floor((((float)(*((int16_t *)mavlink_parameters_list[i].pparam) /
                       16384.0)) *
              10000) +
             0.5) /
       10000.0),
...
void mavlink_set_param_Q14(mavlink_param_union_t setting, int16_t i) {
  ...
  *((int16_t *)mavlink_parameters_list[i].pparam) =
      (int16_t)(setting.param_float * 16384.0);
}

void mavlink_send_param_pwtrim(int16_t i) {
  // Check that the size of the udb_pwtrim array is not exceeded
  if (mavlink_parameters_list[i].pparam >=
      (uint8_t *)(&udb_pwTrim[0] + (sizeof(udb_pwTrim[0]) * NUM_INPUTS)))
    return;
...
void mavlink_set_param_pwtrim(mavlink_param_union_t setting, int16_t i) {
  ...
  *((int16_t *)mavlink_parameters_list[i].pparam) =
      (int16_t)(setting.param_float * 2.0);
}
...
void mavlink_send_param_int16(int16_t i) {
  ...
  mavlink_msg_param_value_send(MAVLINK_COMM_0, mavlink_parameters_list[i].name,
                               param.param_float, MAVLINK_TYPE_INT32_T,
                               count_of_parameters_list,
                               i);  // 16384.0 is RMAX defined as a float.
}
...
void mavlink_send_int_circular(int16_t i) {
  ...
  deg_angle.WW =
      __builtin_mulss(deg_angle._.W0, (int16_t)(RMAX * 180.0 / 256.0));

  deg_angle.WW >>= 5;
  if (deg_angle._.W0 > 0x8000)
    deg_angle._.W1++;  // Take care of the rounding error
...
void mavlink_set_int_circular(mavlink_param_union_t setting, int16_t i) {
  ...
  dec_angle.WW = __builtin_mulss((int16_t)setting.param_int32,
                                 (int16_t)(RMAX * (256.0 / 180.0)));
  dec_angle.WW <<= 9;
  if (dec_angle._.W0 > 0x8000)
...
void mavlink_send_cm_airspeed_in_m(int16_t i) {
  ...
  mavlink_msg_param_value_send(MAVLINK_COMM_0, mavlink_parameters_list[i].name,
                               param.param_float, MAVLINK_TYPE_FLOAT,
                               count_of_parameters_list, i);
}
...
void mavlink_set_cm_airspeed_from_m(mavlink_param_union_t setting, int16_t i) {
  ...
  *((int16_t *)mavlink_parameters_list[i].pparam) =
      (int16_t)(setting.param_float * 100.0);
}
...
void mavlink_set_dm_airspeed_from_m(mavlink_param_union_t setting, int16_t i) {
  ...
  *((int16_t *)mavlink_parameters_list[i].pparam) =
      (int16_t)(setting.param_float * 10.0);
}
...
void mavlink_send_dcm_angle(int16_t i) {
  ...
  deg_angle.WW = __builtin_mulss(
      deg_angle._.W0, (int16_t)(57.3 * 16.0));  //(RMAX * 180.0 / 256.0));
  deg_angle.WW >>= 2;
  if (deg_angle._.W0 > 0x8000)
    deg_angle._.W1++;  // Take care of the rounding error
...
void mavlink_set_dcm_angle(mavlink_param_union_t setting, int16_t i) {
  ...
  union longww dec_angle;
  dec_angle.WW =
      __builtin_mulss((int16_t)setting.param_int32,
                      (RMAX * (16.0 / 57.3)));  //(int16_t)(RMAX * 64 / 57.3)
  dec_angle.WW <<= 12;
  if (dec_angle._.W0 > 0x8000)
    dec_angle.WW += 0x8000;  // Take care of the rounding error
  *((int16_t *)mavlink_parameters_list[i].pparam) = dec_angle._.W1;
...
void mavlink_send_frame_anglerate(int16_t i) {
  ...
  //	deg_angle.WW = __builtin_mulss(deg_angle._.W0, 40);
  deg_angle.WW = __builtin_mulss(
      deg_angle._.W0, (int16_t)(57.3 * 40.0));  //(RMAX * 180.0 / 256.0));
  deg_angle.WW <<= 2;
  if (deg_angle._.W0 > 0x8000)
    deg_angle._.W1++;  // Take care of the rounding error
...
void mavlink_set_frame_anglerate(mavlink_param_union_t setting, int16_t i) {
  ...
  dec_angle.WW =
      __builtin_mulss((int16_t)setting.param_int32,
                      (128.0 * 7.15));  //(int16_t)(RMAX * 128 / (57.3 * 40.0))
  dec_angle.WW <<= 9;
  if (dec_angle._.W0 > 0x8000)
    dec_angle.WW += 0x8000;  // Take care of the rounding error
  *((int16_t *)mavlink_parameters_list[i].pparam) = dec_angle._.W1;
...
boolean mavlink_check_target(uint8_t target_system, uint8_t target_component) {
  if ((target_system == mavlink_system.sysid)
      // QgroundControl sends parameter refresh list to component 25
      // (regardless)
      // But  "Transmit" of parameter updates are sent using a specific
      // component ID of 1 by QGroundControl.
      // Only use mavlink_check_target if you expect all of the sysid, and
      // compid to be correct.
      &&
...
// Portions of the following code in handlesmessage() are templated off source
// code written by James Goppert for the
// ArdupilotMega, and are used by his kind permission and also in accordance
// with the GPS V3 licensing
// of that code.

void handleMessage(void)
    // This is the main routine for taking action against a parsed message from
    // the GCS
{
  ...
  switch (handle_msg->msgid) {
    case MAVLINK_MSG_ID_REQUEST_DATA_STREAM: {
      ...
      // QgroundControl sends data stream request to component ID 1, which is
      // not our component for UDB.
      ...
      if (packet.req_stream_id == MAV_DATA_STREAM_ALL) {
        // Warning: mavproxy automatically sets all.  Do not include all here,
        // it will overide defaults.
    ...
    case MAVLINK_MSG_ID_COMMAND_LONG: {
      ...
      // if (mavlink_check_target(packet.target,packet.target_component) ==
      // false) break;
        ...
        case MAV_CMD_PREFLIGHT_STORAGE:
          if (packet.param1 == MAV_PFS_CMD_WRITE_ALL) {
            if (packet.param2 == MAV_PFS_CMD_WRITE_ALL)
              data_services_save_all(
                  STORAGE_FLAG_STORE_CALIB | STORAGE_FLAG_STORE_WAYPOINTS,
                  &preflight_storage_complete_callback);
            else
              data_services_save_all(STORAGE_FLAG_STORE_CALIB,
                                     &preflight_storage_complete_callback);
          } else if (packet.param1 == MAV_PFS_CMD_READ_ALL) {
            if (packet.param2 == MAV_PFS_CMD_READ_ALL)
              data_services_load_all(
                  STORAGE_FLAG_STORE_CALIB | STORAGE_FLAG_STORE_WAYPOINTS,
                  &preflight_storage_complete_callback);
            else
              data_services_load_all(STORAGE_FLAG_STORE_CALIB,
                                     &preflight_storage_complete_callback);
          } else
            command_ack(packet.command, MAV_CMD_ACK_ERR_NOT_SUPPORTED);
          break;
        ...
        case MAV_CMD_PREFLIGHT_STORAGE_ADVANCED:
          switch ((uint16_t)packet.param1) {
            case MAV_PFS_CMD_CLEAR_SPECIFIC:
              storage_clear_area(packet.param2,
                                 &preflight_storage_complete_callback);
              break;
            case MAV_PFS_CMD_WRITE_SPECIFIC:
              data_services_save_specific(packet.param2,
                                          &preflight_storage_complete_callback);
              break;
            case MAV_PFS_CMD_READ_SPECIFIC:
              data_services_load_specific(packet.param2,
                                          &preflight_storage_complete_callback);
              break;
            default:
              command_ack(packet.command, MAV_CMD_ACK_ERR_NOT_SUPPORTED);
      ...
      /*
                      case MAVLINK_MSG_ID_ACTION:
                              // send_text((uint8_t*) "Action: Specific Action
         Required\r\n");
                              // decode
                              mavlink_action_t packet;
                              mavlink_msg_action_decode(handle_msg, &packet);
                              if
         (mavlink_check_target(packet.target,packet.target_component) == false)
         break;
...
case MAVLINK_MSG_ID_WAYPOINT_REQUEST:
        ...
        {
                send_text((uint8_t*)"ID WAYPOINT REQUEST not valid, no longer
sending\r\n");
                break;
        }
        // decode
...
// case CMD_LOITER_TIME: // loiter
                //orbit = get(PARAM_WP_RADIUS); // XXX setting loiter radius as
...
// case CMD_TAKEOFF: // takeoff
        ...
        //float yaw_dir = 0; // yaw orientation in radians, 0 = north XXX: what
does this do?
...
case MAVLINK_MSG_ID_WAYPOINT_CLEAR_ALL:
        ...
        //if
(mavlink_check_target(packet.target_system,packet.target_component)) break;
        ...
        //{
                ...
                //{
                        //tell_command.lng = 1.0e7*ToDeg(packet.x/
                                        //(radius_of_earth*cos(ToRad(home.lat/1.0e7))))
+ home.lng;
                        //tell_command.lat =
1.0e7*ToDeg(packet.y/radius_of_earth) + home.lat;
...
*****/
    ...
    case MAVLINK_MSG_ID_PARAM_SET: {
      ...
      if (mavlink_check_target(packet.target_system, packet.target_component) ==
          true) {
        send_text(
            (uint8_t *)"failed target system check on parameter set \r\n");
        break;
      } else {
        // set parameter
        ...
        for (i = 0; i < count_of_parameters_list; i++) {
          ...
          if (!strcmp(key, (const char *)mavlink_parameters_list[i].name)) {
            ...
            if ((mavlink_parameters_list[i].readonly == false) &&
              ...
              mavlink_parameter_parsers
                  [mavlink_parameters_list[i].udb_param_type]
                      .set_param(param, i);
              // After setting parameter, re-send it to GCS as acknowledgement
              // of success.
    ...
    case MAVLINK_MSG_ID_FLEXIFUNCTION_BUFFER_FUNCTION: {
      ...
      flexiFunction_write_buffer_function(&packet.data[0], packet.func_index,
                                          packet.data_address, packet.data_size,
                                          packet.func_count);
      break;
    }
    case MAVLINK_MSG_ID_FLEXIFUNCTION_DIRECTORY: {
      ...
      flexiFunction_write_directory(packet.directory_type, packet.start_index,
                                    packet.count, packet.directory_data);
      break;
    }
...
// Decide whether it the correct moment to send a given telemetry update,
...
#else
{
  ...
  uint8_t mavlink_base_mode;     // System mode, see MAV_MODE ENUM in
                                 // mavlink/include/mavlink_types.h
  uint32_t mavlink_custom_mode;  // Custom Status mode specific to the UDB /
                                 // MatrixPilot
  int32_t lat, lon, alt, relative_alt = 0;
  uint16_t mavlink_heading = 0;

  enum MAV_CUSTOM_UDB_MODE_FLAG {
    MAV_CUSTOM_UDB_MODE_MANUAL = 1,  // Manual Mode. MatrixPilot passes all PWM
                                     // signals from Receiver straight out to
                                     // Servos
    MAV_CUSTOM_UDB_MODE_STABILIZE = 2,  // Stabilzed Mode. MatrixPilot assists
                                        // in flying plane. Pilot still commands
                                        // plane from transmitter.
    MAV_CUSTOM_UDB_MODE_AUTONOMOUS = 3,  // Autonmous Mode. Plane is primarily
                                         // flying using waypoints or Logo
                                         // flight language (alghouth pilot can
                                         // mix in control from transmitter).
    MAV_CUSTOM_UDB_MODE_RTL = 4,  // Return to Launch or Failsafe Mode. This
                                  // mode means plane has lost contact with
                                  // pilot's control transmitter.
  };

  uint8_t spread_transmission_load = 0;  // Used to spread sending of different
                                         // message types over a period of 1
                                         // second.
  ...
  // Note that message types are arranged in order of importance so that if the
  // serial buffer fills up,
  // critical message types are more likely to still be transmitted.

  // HEARTBEAT
  spread_transmission_load = 1;

  if (mavlink_frequency_send(MAVLINK_RATE_HEARTBEAT,
                             mavlink_counter_40hz + spread_transmission_load)) {
    if (flags._.GPS_steering == 0 && flags._.pitch_feedback == 0) {
      ...
      mavlink_base_mode =
          MAV_MODE_GUIDED_ARMED | MAV_MODE_FLAG_CUSTOM_MODE_ENABLED;
      mavlink_custom_mode = MAV_CUSTOM_UDB_MODE_STABILIZE;
    } else if (flags._.GPS_steering == 1 && flags._.pitch_feedback == 1 &&
               udb_flags._.radio_on == 1) {
      mavlink_base_mode =
          MAV_MODE_AUTO_ARMED | MAV_MODE_FLAG_CUSTOM_MODE_ENABLED;
      mavlink_custom_mode = MAV_CUSTOM_UDB_MODE_AUTONOMOUS;
    } else if (flags._.GPS_steering == 1 && flags._.pitch_feedback == 1 &&
               udb_flags._.radio_on == 0) {
      mavlink_base_mode =
          MAV_MODE_AUTO_ARMED |
          MAV_MODE_FLAG_CUSTOM_MODE_ENABLED;  // Return to Landing (lost contact
                                              // with transmitter)
    ...
    mavlink_msg_heartbeat_send(MAVLINK_COMM_0, MAV_TYPE_FIXED_WING,
                               MAV_AUTOPILOT_UDB, mavlink_base_mode,
                               mavlink_custom_mode, MAV_STATE_ACTIVE);
    // mavlink_msg_heartbeat_send(mavlink_channel_t chan, uint8_t type, uint8_t
    // autopilot, uint8_t base_mode, uint32_t custom_mode, uint8_t
    // system_status)
  }

  spread_transmission_load = 4;
  if (mavlink_frequency_send(streamRates[MAV_DATA_STREAM_RAW_SENSORS],
                             mavlink_counter_40hz + spread_transmission_load)) {
    ...
    if (gps_nav_valid())
      ...
      gps_fix_type = 0;
    mavlink_msg_gps_raw_int_send(MAVLINK_COMM_0, usec, gps_fix_type, lat_gps.WW,
                                 long_gps.WW, alt_sl_gps.WW, hdop, 65535,
                                 sog_gps.BB, cog_gps.BB, svs);
  }
  ...
  if (mavlink_frequency_send(streamRates[MAV_DATA_STREAM_POSITION],
                             mavlink_counter_40hz + spread_transmission_load)) {
    accum_A_long.WW =
        IMUlocationy._.W1 +
        (int32_t)(lat_origin.WW / 90.0);  // meters North from Equator
    ...
    if (cos_lat == 0) {
      ...
      accum_A_long.WW = accum_A_long.WW * 16384;  // Compiler uses (shift left
                                                  // 14) for this multiplication
      accum_B_long.WW = (accum_A_long.WW + 8192) /
                        cos_lat;  // 8192 improves rounding accuracy
      lon = long_origin.WW + (accum_B_long.WW * 90);  // degrees
    }
    accum_A_long.WW = IMUlocationz._.W1;
    relative_alt = accum_A_long.WW * 1000;
    alt = relative_alt + (alt_origin.WW * 10);  // In millimeters; more accurate
                                                // if used IMUlocationz._.W0

    mavlink_heading =
        get_geo_heading_angle() *
        100;  // mavlink global position expects heading value x 100
    mavlink_msg_global_position_int_send(
        MAVLINK_COMM_0, msec, lat, lon, alt, relative_alt, -IMUvelocityy._.W1,
        IMUvelocityx._.W1,
        -IMUvelocityz._.W1,  //  IMUVelocity  normal units are in cm / second
        mavlink_heading);    // heading should be from 0 to 35999 meaning 0 to
                             // 359.99 degrees.
    // mavlink_msg_global_position_int_send(mavlink_channel_t chan, uint32_t
    // time_boot_ms, int32_t lat, int32_t lon, int32_t alt,
  ...
  if (mavlink_frequency_send(streamRates[MAV_DATA_STREAM_POSITION],
    ...
    accum = rect_to_polar16(
        &matrix_accum);  // binary angle (0 to 65536 = 360 degrees)
    ...
    earth_pitch_velocity = (earth_pitch - previous_earth_pitch) *
                           streamRates[MAV_DATA_STREAM_POSITION];
    earth_roll_velocity = (earth_roll - previous_earth_roll) *
                          streamRates[MAV_DATA_STREAM_POSITION];
    earth_yaw_velocity = (earth_yaw - previous_earth_yaw) *
                         streamRates[MAV_DATA_STREAM_POSITION];
    ...
    mavlink_msg_attitude_send(MAVLINK_COMM_0, msec, earth_roll, earth_pitch,
                              earth_yaw, earth_roll_velocity,
                              earth_pitch_velocity, earth_yaw_velocity);
    // mavlink_msg_attitude_send(mavlink_channel_t chan, uint32_t time_boot_ms,
  ...
  if (mavlink_frequency_send(streamRates[MAV_DATA_STREAM_POSITION],
    ...
    mavlink_msg_vfr_hud_send(
        MAVLINK_COMM_0, (float)(air_speed_3DIMU / 100.0),
        (float)(ground_velocity_magnitudeXY / 100.0), (int16_t)mavlink_heading,
        (uint16_t)(((float)((udb_pwOut[THROTTLE_OUTPUT_CHANNEL]) -
                            udb_pwTrim[THROTTLE_INPUT_CHANNEL]) *
                    100.0) /
                   (float)(pwOut_max - udb_pwTrim[THROTTLE_INPUT_CHANNEL])),
        ((float)(IMUlocationz._.W1 + (alt_origin.WW / 100.0))),
        (float)-IMUvelocityz._.W1);
    // void mavlink_msg_vfr_hud_send(mavlink_channel_t chan, float airspeed,
    // float groundspeed, int16_t heading, uint16_t throttle, float alt, float
    // climb)
  }
#endif
  ...
  if (mavlink_frequency_send(MAVLINK_RATE_SYSTEM_STATUS,
                             mavlink_counter_40hz + spread_transmission_load)) {
    mavlink_msg_sys_status_send(
        MAVLINK_COMM_0, 0,       // Sensors fitted
        0,                       // Sensors enabled
        0,                       // Sensor health
        udb_cpu_load() * 10, 0,  // Battery voltage in mV
        0,                       // Current
        0,  // Percentage battery remaining 100 percent is 1000
        r_mavlink_status.packet_rx_drop_count, 0,  // errors_comm
        0,                                         // errors_count1
        0,                                         // errors_count2
        0,                                         // errors_count3
        0);                                        // errors_count4

    // mavlink_msg_sys_status_send(mavlink_channel_t chan, uint32_t
    // onboard_control_sensors_present, uint32_t
    // onboard_control_sensors_enabled,
    // uint32_t onboard_control_sensors_health, uint16_t load, uint16_t
    // voltage_battery, int16_t current_battery, int8_t battery_remaining,
    ...
    // Sensor Indices: 0: 3D gyro, 1: 3D acc, 2: 3D mag, 3: absolute pressure,
    // 4: differential pressure, 5: GPS, 6: optical flow, 7: computer vision
    // position, 8: laser based position, 9: external ground-truth (Vicon or
    // Leica). Controllers: 10: 3D angular rate control 11: attitude
    // stabilization, 12: yaw position, 13: z/altitude control, 14: x/y position
    // control, 15: motor outputs / control
  }

  // RC CHANNELS
  // Channel values shifted left by 1, to divide by two, so values reflect PWM
  // pulses in microseconds.
  // mavlink_msg_rc_channels_raw_send(mavlink_channel_t chan, uint16_t
  // chan1_raw, uint16_t chan2_raw,
  //	uint16_t chan3_raw, uint16_t chan4_raw, uint16_t chan5_raw, uint16_t
  //chan6_raw, uint16_t chan7_raw,
  //	uint16_t chan8_raw, uint8_t rssi)
  spread_transmission_load = 24;
  if (mavlink_frequency_send(streamRates[MAV_DATA_STREAM_RAW_SENSORS],
                             mavlink_counter_40hz + spread_transmission_load)) {
    mavlink_msg_rc_channels_raw_send(
        MAVLINK_COMM_0, msec, (uint16_t)((udb_pwIn[0]) >> 1),
        (uint16_t)((udb_pwIn[1]) >> 1), (uint16_t)((udb_pwIn[2]) >> 1),
        (uint16_t)((udb_pwIn[3]) >> 1), (uint16_t)((udb_pwIn[4]) >> 1),
        (uint16_t)((udb_pwIn[5]) >> 1), (uint16_t)((udb_pwIn[6]) >> 1),
  ...
  // mavlink_msg_rc_channels_raw_send(mavlink_channel_t chan, uint32_t
  // time_boot_ms, uint8_t port, uint16_t chan1_raw, uint16_t chan2_raw,
  // uint16_t chan3_raw, uint16_t chan4_raw,
  //	uint16_t chan5_raw, uint16_t chan6_raw, uint16_t chan7_raw, uint16_t
  //chan8_raw, uint8_t rssi)
  // RAW SENSORS - ACCELOREMETERS and GYROS
  // It is expected that these values are graphed to allow users to check basic
  // sensor operation,
  // and to graph noise on the signals. As this code if for testing and graphing
  // basic hardware, it uses
  // UDB conventions coordinate conventions for X,Y and Z axis rather than
  // MAVLink conventions.
  // See:- http://code.google.com/p/gentlenav/wiki/UDBCoordinateSystems and the
  // "Aviation Convention" diagram.
  ...
  if (mavlink_frequency_send(streamRates[MAV_DATA_STREAM_RAW_SENSORS],
    ...
    mavlink_msg_raw_imu_send(
        MAVLINK_COMM_0, usec, (int16_t)udb_xaccel.value,
        (int16_t)udb_yaccel.value, (int16_t) - udb_zaccel.value,
        (int16_t) - udb_xrate.value, (int16_t) - udb_yrate.value,
        (int16_t) - udb_zrate.value, (int16_t)magFieldRaw[0],
        (int16_t)magFieldRaw[1], (int16_t)magFieldRaw[2]);
#else                     // magnetometer is not connected
    mavlink_msg_raw_imu_send(
        MAVLINK_COMM_0, usec, (int16_t)udb_xaccel.value,
        (int16_t)udb_yaccel.value, (int16_t) - udb_zaccel.value,
        (int16_t) - udb_xrate.value, (int16_t) - udb_yrate.value,
        (int16_t) - udb_zrate.value, (int16_t)0, (int16_t)0,
        (int16_t)0);  // zero as mag not connected.
#endif
    // mavlink_msg_raw_imu_send(mavlink_channel_t chan, uint64_t time_usec,
    // int16_t xacc, int16_t yacc, int16_t zacc,
    //		int16_t xgyro, int16_t ygyro, int16_t zgyro, int16_t xmag, int16_t
    //ymag, int16_t zmag)
  }
  ...
  if (mavlink_frequency_send(streamRates[MAV_DATA_STREAM_EXTRA2],
                             mavlink_counter_40hz + spread_transmission_load)) {
    mavlink_msg_altitudes_send(MAVLINK_COMM_0, msec, alt_sl_gps.WW,
                               relative_alt, 0, 0, 0, 0);
    // static inline void mavlink_msg_altitudes_send(mavlink_channel_t chan,
    // uint32_t time_boot_ms, int32_t alt_gps, int32_t alt_imu, int32_t
    // alt_barometric, int32_t alt_optical_flow, int32_t alt_range_finder,
    // int32_t alt_extra)
  }
  ...
  if (mavlink_frequency_send(streamRates[MAV_DATA_STREAM_EXTRA2],
    ...
    // mavlink_msg_airspeeds_send(mavlink_channel_t chan, uint32_t time_boot_ms,
    // int16_t airspeed_imu, int16_t airspeed_pitot, int16_t airspeed_hot_wire,
    // int16_t airspeed_ultrasonic, int16_t aoa, int16_t aoy)
  }

  // SEND SERIAL_UDB_EXTRA (SUE) VIA MAVLINK FOR BACKWARDS COMPATIBILITY with
  // FLAN.PYW (FLIGHT ANALYZER)
  // The MAVLink messages for this section of code are unique to MatrixPilot and
  // are defined in matrixpilot.xml
  spread_transmission_load = 10;
  if (mavlink_frequency_send(
          streamRates[MAV_DATA_STREAM_EXTRA1],
          mavlink_counter_40hz +
              spread_transmission_load))  // SUE code historically ran at 8HZ
  {
    switch (mavlink_sue_telemetry_counter) {
      case 8:
        mavlink_msg_serial_udb_extra_f14_send(
            MAVLINK_COMM_0, WIND_ESTIMATION, GPS_TYPE, DEADRECKONING,
            BOARD_TYPE, AIRFRAME_TYPE, get_reset_flags(), trap_flags,
            trap_source, osc_fail_count, CLOCK_CONFIG, FLIGHT_PLAN_TYPE);
        mavlink_sue_telemetry_counter--;
        break;
      case 7:
        mavlink_msg_serial_udb_extra_f15_send(
            MAVLINK_COMM_0, (uint8_t *)ID_VEHICLE_MODEL_NAME,
      ...
      case 6:
        ...
        break;
      case 5:
        mavlink_msg_serial_udb_extra_f4_send(
            MAVLINK_COMM_0, ROLL_STABILIZATION_AILERONS,
            ROLL_STABILIZATION_RUDDER, PITCH_STABILIZATION,
            YAW_STABILIZATION_RUDDER, YAW_STABILIZATION_AILERON,
            AILERON_NAVIGATION, RUDDER_NAVIGATION, ALTITUDEHOLD_STABILIZED,
            ALTITUDEHOLD_WAYPOINT, RACING_MODE);
        mavlink_sue_telemetry_counter--;
        break;
      case 4:
        mavlink_msg_serial_udb_extra_f5_send(
            MAVLINK_COMM_0, YAWKP_AILERON, YAWKD_AILERON, ROLLKP, ROLLKD,
            YAW_STABILIZATION_AILERON, AILERON_BOOST);
        mavlink_sue_telemetry_counter--;
        break;
      case 3:
        mavlink_msg_serial_udb_extra_f6_send(MAVLINK_COMM_0, PITCHGAIN, PITCHKD,
                                             RUDDER_ELEV_MIX, ROLL_ELEV_MIX,
                                             ELEVATOR_BOOST);
        mavlink_sue_telemetry_counter--;
        break;
      case 2:
        mavlink_msg_serial_udb_extra_f7_send(
            MAVLINK_COMM_0, YAWKP_RUDDER, YAWKD_RUDDER, ROLLKP_RUDDER,
            ROLLKD_RUDDER, RUDDER_BOOST, RTL_PITCH_DOWN);
        mavlink_sue_telemetry_counter--;
        break;
      case 1:
        mavlink_msg_serial_udb_extra_f8_send(
            MAVLINK_COMM_0, HEIGHT_TARGET_MAX, HEIGHT_TARGET_MIN,
            ALT_HOLD_THROTTLE_MIN, ALT_HOLD_THROTTLE_MAX, ALT_HOLD_PITCH_MIN,
            ALT_HOLD_PITCH_MAX, ALT_HOLD_PITCH_HIGH);
        mavlink_sue_telemetry_counter--;
        break;
      default: {
        if (mavlink_sue_telemetry_f2_a == true) {
          ...
          // It allows telemetry to have a time reference when the GPS time
          // reference is lost
          // Note this does increment the official Time of Week (TOW) for the
          // entire system.
          // It is not changed for now, to preserve close compatibility with
          // origin SERIAL_UDB_EXTRA code.
          if (tow.WW > 0) tow.WW += 250;

          if (flags._.f13_print_req == 1) {
            // The F13 line of telemetry is printed just once  when origin has
            // been captured after GPS lock
            mavlink_msg_serial_udb_extra_f13_send(MAVLINK_COMM_0, week_no.BB,
                                                  lat_origin.WW, long_origin.WW,
                                                  alt_origin.WW);
          ...
          mavlink_msg_serial_udb_extra_f2_a_send(
              MAVLINK_COMM_0, tow.WW,
              ((udb_flags._.radio_on << 2) + (dcm_flags._.nav_capable << 1) +
               flags._.GPS_steering),
              lat_gps.WW, long_gps.WW, alt_sl_gps.WW, waypointIndex, rmat[0],
              rmat[1], rmat[2], rmat[3], rmat[4], rmat[5], rmat[6], rmat[7],
              rmat[8], (uint16_t)cog_gps.BB, sog_gps.BB,
              (uint16_t)udb_cpu_load(), voltage_milis.BB, air_speed_3DIMU,
              estimatedWind[0], estimatedWind[1], estimatedWind[2],
              magFieldEarth[0], magFieldEarth[1], magFieldEarth[2], svs, hdop);
#else
          mavlink_msg_serial_udb_extra_f2_a_send(
              MAVLINK_COMM_0, tow.WW,
              ((udb_flags._.radio_on << 2) + (dcm_flags._.nav_capable << 1) +
              ...
              rmat[8], (uint16_t)cog_gps.BB, sog_gps.BB,
              (uint16_t)udb_cpu_load(), voltage_milis.BB, air_speed_3DIMU,
              estimatedWind[0], estimatedWind[1], estimatedWind[2], 0, 0, 0,
              svs, hdop);
#endif

          // Save  pwIn and PwOut buffers for sending next time around in f2_b
          // format message
          int16_t i;
          for (i = 0; i <= (NUM_INPUTS > MAVLINK_SUE_CHANNEL_MAX_SIZE
                                ? MAVLINK_SUE_CHANNEL_MAX_SIZE
                                : NUM_INPUTS);
               i++)
            pwIn_save[i] = udb_pwIn[i];
          for (i = 0; i <= (NUM_OUTPUTS > MAVLINK_SUE_CHANNEL_MAX_SIZE
                                ? MAVLINK_SUE_CHANNEL_MAX_SIZE
                                : NUM_OUTPUTS);
               i++)
...
#if (RECORD_FREE_STACK_SPACE == 1)
          int16_t stack_free =
              (int16_t)(4096 - maxstack);  // This is actually wrong for the
                                           // UDB4, but currently left the same
                                           // as for telemetry.c
#else
          ...
          mavlink_msg_serial_udb_extra_f2_b_send(
              MAVLINK_COMM_0, tow.WW, pwIn_save[1], pwIn_save[2], pwIn_save[3],
              pwIn_save[4], pwIn_save[5], pwIn_save[6], pwIn_save[7],
              pwIn_save[8], pwIn_save[9], pwIn_save[10], pwOut_save[1],
              pwOut_save[2], pwOut_save[3], pwOut_save[4], pwOut_save[5],
              pwOut_save[6], pwOut_save[7], pwOut_save[8], pwOut_save[9],
              pwOut_save[10], IMUlocationx._.W1, IMUlocationy._.W1,
              IMUlocationz._.W1, flags.WW,
#if (SILSIM != 1)
              osc_fail_count,
#else
              0,
#endif
              IMUvelocityx._.W1, IMUvelocityy._.W1, IMUvelocityz._.W1, goal.x,
              goal.y, goal.height, stack_free);
        }
      }
    }
  }
  ...
  if (mavlink_flags.mavlink_send_variables == 1) {
    if (send_variables_counter < count_of_parameters_list) {
      mavlink_parameter_parsers
          [mavlink_parameters_list[send_variables_counter].udb_param_type]
              .send_param(send_variables_counter);
  ...
  if (mavlink_flags.mavlink_send_specific_variable == 1) {
    mavlink_parameter_parsers
        [mavlink_parameters_list[send_by_index].udb_param_type]
            .send_param(send_by_index);
    mavlink_flags.mavlink_send_specific_variable = 0;
  }

#if (FLIGHT_PLAN_TYPE == \
     FP_WAYPOINTS)  // LOGO_WAYPOINTS cannot be uploaded / downloaded
...
{
        if (mavlink_flags.mavlink_sending_waypoints ||
mavlink_flags.mavlink_receiving_waypoints)
        {
...
if (mavlink_flags.mavlink_send_waypoint_count == 1)
{
        ...
        mavlink_msg_waypoint_count_send(MAVLINK_COMM_0,
mavlink_waypoint_dest_sysid, mavlink_waypoint_dest_compid, number_of_waypoints);
...
if (mavlink_flags.mavlink_send_specific_waypoint == 1)
{
                ...
                mavlink_msg_waypoint_send(mavlink_channel_t chan, uint8_t
target_system, uint8_t target_component, \
                        uint16_t seq, uint8_t frame, uint8_t command, uint8_t
current, uint8_t autocontinue, \
...
\
                ...
                //accum_long = IMUlocationy._.W1 + (lat_origin.WW / 90); //
meters North from Equator
                //lat_float  = (float)((accum_long * 90) / 10000000.0); //
degrees North from Equator
                //lon_float = (float)((float) long_origin.WW  +
((float)(IMUlocationx._.W1) * 90.0) / (float)(cos_lat / 16384.0)) / 10000000.0;
                //	extern struct relWaypointDef wp_to_relative(struct
waypointDef wp);
                //struct relWaypointDef current_waypoint =
wp_to_relative(waypoints[waypointIndex]);
                alt_float =  ((float)(IMUlocationz._.W1)) +
(float)(alt_origin.WW / 100.0);
                mavlink_msg_waypoint_send(MAVLINK_COMM_0,
mavlink_waypoint_dest_sysid, mavlink_waypoint_dest_compid, \
                        mavlink_waypoint_requested_sequence_number,
mavlink_waypoint_frame, 0, mavlink_waypoint_current, true, \
                        0.0, 0.0, 0.0, 0.0, \
                        2.0, 54.0, 500.0);
...
***********************/
  ...
  switch (flexiFunctionState) {
    case FLEXIFUNCTION_BUFFER_FUNCTION_ACKNOWLEDGE:
      mavlink_msg_flexifunction_buffer_function_ack_send(
          MAVLINK_COMM_0, 0, 0, flexifunction_ref_index,
          flexifunction_ref_result);
      flexiFunctionState = FLEXIFUNCTION_WAITING;
      break;
    case FLEXIFUNCTION_INPUT_DIRECTORY_ACKNOWLEDGE:
      mavlink_msg_flexifunction_directory_ack_send(
          MAVLINK_COMM_0, 0, 0, 1, 0, 32, flexifunction_ref_result);
      flexiFunctionState = FLEXIFUNCTION_WAITING;
      break;
    case FLEXIFUNCTION_OUTPUT_DIRECTORY_ACKNOWLEDGE:
      mavlink_msg_flexifunction_directory_ack_send(
          MAVLINK_COMM_0, 0, 0, 0, 0, 32, flexifunction_ref_result);
      flexiFunctionState = FLEXIFUNCTION_WAITING;
      break;
    case FLEXIFUNCTION_COMMAND_ACKNOWLEDGE:
      mavlink_msg_flexifunction_command_ack_send(
          MAVLINK_COMM_0, flexifunction_ref_command, flexifunction_ref_result);
      flexiFunctionState = FLEXIFUNCTION_WAITING;
      break;
  }
#endif  //#if (USE_FLEXIFUNCTION_MIXING == 1)
  ...
  if (mavlink_send_command_ack == true) {
    mavlink_msg_command_ack_send(MAVLINK_COMM_0, mavlink_command_ack_command,
                                 mavlink_command_ack_result);
