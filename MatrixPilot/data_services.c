...
typedef enum {
  ...
  DATA_SERVICE_STATE_READ_DONE,  // Done reading, check and then commit it to
                                 // ram.
...
void data_services_init(void) {
  ...
  if ((data_service_event_handle = register_event(&data_services)) ==
      INVALID_HANDLE)
    return;
  ...
  while (data_services_table_index < mavlink_parameter_block_count) {
    if (storage_services_started()) {
      if (storage_check_area_exists(
              mavlink_parameter_blocks[data_services_table_index]
                  .data_storage_area,
...
uint16_t data_services_calc_item_size(uint16_t table_index) {
  ...
  uint16_t start_index =
      mavlink_parameter_blocks[table_index].block_start_index;
...
void data_services_read_index(void) {
  ...
  uint16_t service_flags =
      mavlink_parameter_blocks[data_services_table_index].data_storage_flags;

  // Check the serialise flags to see if this table entry should be loaded
  if ((service_flags & data_services_serialize_flags) |
    ...
    uint16_t type =
        DATA_STORAGE_CHECKSUM_STRUCT;  // mavlink_parameter_blocks[data_services_table_index].data_type;

    // TODO: Check here if data handle is ok

    if (type == DATA_STORAGE_CHECKSUM_STRUCT) {
      if (storage_read(handle, data_services_buffer, size,
                       &data_services_read_callback) == true) {
    ...
    if (data_services_do_all_areas == true)
      ...
      if (data_services_user_callback != NULL)
        data_services_user_callback(false);
      data_services_user_callback = NULL;
      data_service_state = DATA_SERVICE_STATE_WAITING;
    }
  }
...
boolean data_services_load_specific(uint16_t data_storage_handle,
  ...
  data_services_table_index =
      data_services_get_table_index(data_storage_handle);
  if (data_services_table_index == INVALID_HANDLE) return false;

  data_services_user_callback = pcallback;
  data_services_do_all_areas = false;  // One area only
  data_services_serialize_flags =
      STORAGE_FLAG_ALL;  // Flag to read regardless of flags
  data_service_state = DATA_SERVICE_STATE_READ;
...
void data_services_read_done(void) {
  ...
  if (mavlink_parameter_blocks[data_services_table_index].ploadCallback !=
      NULL) {
...
void data_services_read_callback(boolean success) {
  ...
  else if (data_services_do_all_areas == true) {
    data_services_table_index++;
    data_service_state = DATA_SERVICE_STATE_READ;
  } else {
...
uint16_t serialise_items_to_buffer(uint16_t table_index) {
  ...
  for (item_index = 0; item_index < item_count; item_index++) {
    ...
    if ((buffer_index + item_size) > DATA_SERVICE_BUFFER_SIZE) return 0;
...
uint16_t data_services_get_table_index(uint16_t data_storage_handle) {
  ...
  for (index = 0; index < mavlink_parameter_block_count; index++) {
    if (mavlink_parameter_blocks[index].data_storage_area ==
        data_storage_handle) {
      return index;
    }
  }
...
boolean data_services_save_specific(uint16_t data_storage_handle,
  ...
  data_services_do_all_areas = false;  // One area only
  data_services_serialize_flags =
      STORAGE_FLAG_ALL;  // Flag to write regardless of flags
  ...
  if ((type == DATA_STORAGE_CHECKSUM_STRUCT) &&
      ((data_services_serialize_flags &
        mavlink_parameter_blocks[data_services_table_index]
            .data_storage_flags) ||
       (data_services_serialize_flags & STORAGE_FLAG_ALL))) {
    if (storage_write(handle, data_services_buffer, size,
                      &data_services_write_callback) == true) {
...
void data_services_write_callback(boolean success) {
  if (success) {
    if (data_services_do_all_areas == true) {
      ...
      if (data_services_user_callback != NULL)
        data_services_user_callback(true);
      data_service_state = DATA_SERVICE_STATE_WAITING;
    }
  } else {
    if (data_services_user_callback != NULL) data_services_user_callback(false);
