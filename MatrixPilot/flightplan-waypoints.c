...
// struct manoeuvreDef {int16_t channel; int32_t start_time; int32_t end_time;
// int16_t value; };

// struct vertical_segment { int16_t vz; int16_t alt_start; int16_t alt_end;
// int32_t duration;
//                          boolean is_target_alt; int16_t flags; struct
// manoeuvreDef* manoeuvres; };
//
// int16_t current_manoeuvreValues[NUM_OUTPUTS];

#if (FLIGHT_PLAN_TYPE == FP_WAYPOINTS)

struct relWaypointDef {
  struct relative3D loc;
  int16_t flags;
  struct relative3D viewpoint;
};
struct waypointDef {
  struct waypoint3D loc;
  int16_t flags;
  struct waypoint3D viewpoint;
};
...
uint16_t number_of_waypoints =
    ((sizeof waypoints) / sizeof(struct waypointDef));
#endif
...
const uint8_t wp_inject_byte_order[] = {3,  2,  1,  0,  7,  6,  5,  4,
                                        9,  8,  11, 10, 15, 14, 13, 12,
                                        19, 18, 17, 16, 21, 20};
...
//        {
//            if ( relative_time > manoeuvres[i].start_time && relative_time <
// manoeuvres[i].end_time )
//            {
//                current_manoeuvreValues[manoeuvres[i].channel] =
// manoeuvres[i].value;
//            }
//        }
//    }
...
void next_waypoint(void) {
  ...
  if (waypointIndex == 0) {
    if (numPointsInCurrentSet > 1) {
      struct relWaypointDef previous_waypoint =
          wp_to_relative(currentWaypointSet[numPointsInCurrentSet - 1]);
      struct relWaypointDef current_waypoint =
          wp_to_relative(currentWaypointSet[0]);
    ...
    struct relWaypointDef previous_waypoint =
        wp_to_relative(currentWaypointSet[waypointIndex - 1]);
    struct relWaypointDef current_waypoint =
        wp_to_relative(currentWaypointSet[waypointIndex]);
    set_goal(previous_waypoint.loc, current_waypoint.loc);
    set_camera_view(current_waypoint.viewpoint);
    setBehavior(current_waypoint.flags);
  }
  ...
  if (desired_behavior._.altitude) {
    ...
    if (tofinish_line < WAYPOINT_RADIUS)  // crossed the finish line
    {
      if (desired_behavior._.loiter)
        set_goal(GPSlocation,
                 wp_to_relative(currentWaypointSet[waypointIndex]).loc);
      else
        next_waypoint();
    }
  }
...
void flightplan_live_begin(void) { wp_inject_pos = 0; }
...
//#define NUMBER_SEGMENTS ((sizeof take_off_segment) / sizeof (struct
//vertical_segment))
...
//    {
//        if((current_segment.vz > 0 && current_altitude >
// current_segment.alt_end)
//              || (current_segment.vz < 0 && current_altitude  <
// current_segment.alt_end))
//        {
...
//
//    current_altitude = (int16_t)(((int32_t)(current_segment.vz) * (time -
// init_time - start_time))/1000) + current_segment.alt_start;
//}
