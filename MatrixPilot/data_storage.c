...
// Clear a specific data area by invalidating the checksum and writing it to
// eeprom table
void storage_clear_specific_area(void);

// Callbacks
// The callbacks normally set the status so that the background service routine
// does the work.
void data_storage_format_callback(boolean success);     // format write is
                                                        // completed
void data_storage_init_read_callback(boolean success);  // initialisation read
                                                        // of storage table
...
void data_storage_write_table_callback(boolean success);  // Table write
                                                          // callback

void storage_clear_specific_area_callback(boolean success);  // Clear specific
                                                             // data area
                                                             // finished
                                                             // callback
...
void data_storage_service(void) {
  switch (data_storage_status) {
    case DATA_STORAGE_STATUS_START:
      data_storage_table.table_preamble[0] =
          0x00;  // Make sure memory contents are invalid before reading
      ...
      if (udb_nv_memory_read((uint8_t*)&data_storage_table, 0,
                             sizeof(data_storage_table),
                             &data_storage_init_read_callback) == false)
        return;
      ...
      switch (data_storage_type) {
        ...
        case DATA_STORAGE_SELF_MANAGED:
          if (udb_nv_memory_write(
                  pdata_storage_data,
                  data_storage_table.table[data_storage_handle].data_address,
                  data_storage_size, &storage_write_callback) == false) {
            if (data_storage_user_callback != NULL)
              data_storage_user_callback(false);
            data_storage_status = DATA_STORAGE_STATUS_WAITING;
            return;
    ...
    case DATA_STORAGE_WRITING_DATA_COMPLETE: {
      switch (data_storage_type) {
        case DATA_STORAGE_CHECKSUM_STRUCT: {
          data_storage_header.data_checksum = crc_calculate(
              (uint8_t*)pdata_storage_data, data_storage_data_size);
          data_storage_header.data_handle = data_storage_handle;
          data_storage_header.data_version = 0;
          memcpy(data_storage_header.data_preamble, data_storage_preamble,
                 sizeof(data_storage_preamble));

          if (udb_nv_memory_write(
                  (uint8_t*)&data_storage_header,
                  data_storage_table.table[data_storage_handle].data_address,
                  sizeof(DATA_STORAGE_HEADER),
    ...
    case DATA_STORAGE_READ:
      data_storage_type =
          data_storage_table.table[data_storage_handle].data_type;

      switch (data_storage_type) {
        case DATA_STORAGE_CHECKSUM_STRUCT:
          if (udb_nv_memory_read(
                  (uint8_t*)&data_storage_header,
                  data_storage_table.table[data_storage_handle].data_address,
                  sizeof(DATA_STORAGE_HEADER),
        ...
        case DATA_STORAGE_SELF_MANAGED:
          if (udb_nv_memory_read(
                  pdata_storage_data,
                  data_storage_table.table[data_storage_handle].data_address,
                  data_storage_size, &storage_read_data_callback) == false) {
            if (data_storage_user_callback != NULL)
              data_storage_user_callback(false);
            data_storage_status = DATA_STORAGE_STATUS_WAITING;
          } else
    ...
    case DATA_STORAGE_READ_HEADER_COMPLETE: {
      if (udb_nv_memory_read(
              ...
              data_storage_table.table[data_storage_handle].data_address +
                  sizeof(DATA_STORAGE_HEADER),
              data_storage_data_size, &storage_read_data_callback) == false) {
        if (data_storage_user_callback != NULL)
          data_storage_user_callback(false);
        data_storage_status = DATA_STORAGE_STATUS_WAITING;
      } else
    ...
    case DATA_STORAGE_READ_DATA_COMPLETE: {
      ...
      if (data_storage_type == DATA_STORAGE_CHECKSUM_STRUCT) {
        ...
        if (data_storage_header.data_handle != data_storage_handle)
          success = false;

        // If preamble is incorrect then fail
        if (memcmp(data_storage_header.data_preamble, data_storage_preamble,
                   4) != 0)
          success = false;

        // If checksum is incorrect then fail.

        if (data_storage_header.data_checksum !=
            crc_calculate((uint8_t*)pdata_storage_data, data_storage_data_size))
          success = false;
      }
    ...
    case DATA_STORAGE_AREA_CREATE:
      if ((data_storage_table.table[data_storage_handle].data_address =
               data_storage_find_hole(data_storage_size)) == 0) {
      ...
      data_storage_table.table[data_storage_handle].data_size =
          data_storage_size;
      data_storage_table.table[data_storage_handle].data_type =
          data_storage_type;
      ...
      data_storage_table.table_checksum =
          crc_calculate((uint8_t*)data_storage_table.table,
                        sizeof(*data_storage_table.table));

      // Store the table.  If storage area create fails, put the services in
      // wait.
      if (udb_nv_memory_write((uint8_t*)&data_storage_table, 0,
                              sizeof(data_storage_table),
                              &data_storage_write_table_callback) == false) {
...
void storage_service_trigger(void) { trigger_event(data_storage_event_handle); }
...
boolean data_storage_check_table(void) {
  ...
  if (memcmp(data_storage_table.table_preamble, table_storage_preamble, 4) != 0)
    return false;

  // Calcualte the checksum
  mem_checksum = crc_calculate((uint8_t*)data_storage_table.table,
                               sizeof(*data_storage_table.table));
...
boolean data_storage_format_table(void) {
  ...
  // Calculate the checksum
  mem_counter = crc_calculate((uint8_t*)data_storage_table.table,
                              sizeof(*data_storage_table.table));
  ...
  if (udb_nv_memory_write((uint8_t*)&data_storage_table, 0,
                          sizeof(data_storage_table),
                          &data_storage_format_callback) == false)
    return false;
  return true;
}
...
boolean storage_write(uint16_t data_handle, uint8_t* pwrData, uint16_t size,
  ...
  if (storage_test_handle(data_handle) == false) return false;
  ...
  if (data_storage_table.table[data_handle].data_size != data_storage_size)
    return false;
...
boolean storage_read(uint16_t data_handle, uint8_t* pwrData, uint16_t size,
                     DS_callbackFunc callback) {
  if (data_storage_status != DATA_STORAGE_STATUS_WAITING) return false;
...
boolean storage_check_area_exists(uint16_t data_handle, uint16_t size,
                                  uint16_t type) {
  uint16_t check_size;
  ...
  if (data_storage_table.table[data_handle].data_size != check_size)
    return false;
...
boolean storage_create_area(uint16_t data_handle, uint16_t size, uint16_t type,
                            DS_callbackFunc callback) {
  if (data_storage_status != DATA_STORAGE_STATUS_WAITING) return false;
...
uint16_t data_storage_find_hole(uint16_t data_storage_size) {
  ...
  // go around this loop checking if any of the assigned data areas in the table
  // overlap
  // the proposed data area (lowestAddr-highestAddr).
  // If an overlap is found, move the proposed area to the end of the assigned
  // area, if the
  ...
  do {
    ...
    for (handle = 0; handle <= MAX_DATA_HANDLES; handle++) {
      ...
      if ((scanMinAddr <= lowestAddr) & (scanMaxAddr >= lowestAddr))
        overlap = true;

      // If the highest address is inside the scanned range, data areas overlap
      if ((scanMinAddr <= highestAddr) & (scanMaxAddr >= highestAddr))
        overlap = true;

      // If the scanned range is inside the proposed range, data areas ovverlap
      if ((scanMinAddr >= lowestAddr) & (scanMinAddr <= highestAddr))
        overlap = true;

      // If there has been an overlap of any kind, move the lowest proposed
      // address
      ...
      if (overlap == true) {
        ...
        found_space =
            false;  // flag space not found for this round of the search
      }
    }
  ...
  /*
          for(handle = 0; handle <= DATA_HANDLE_MAX; handle++)
          {
                  ...
                  maxAddr = scanAddr +
     data_storage_table.table[handle].data_size;
...
void storage_clear_specific_area(void) {
  // If the data storage area has not been created, return false
  // TODO: Fix this so that it reports an already clear memory area as correctly
  // cleared
  ...
  if (udb_nv_memory_write(
          (uint8_t*)&data_storage_header,
          data_storage_table.table[data_storage_handle].data_address, size,
          &storage_clear_specific_area_callback) == false) {
    if (data_storage_user_callback != NULL) data_storage_user_callback(false);
    data_storage_status = DATA_STORAGE_STATUS_WAITING;
    return;
  } else
