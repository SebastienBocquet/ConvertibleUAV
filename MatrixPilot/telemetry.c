...
#if (SERIAL_OUTPUT_FORMAT != \
     SERIAL_MAVLINK)  // All MAVLink telemetry code is in MAVLink.c
...
#endif
      fp_high_byte =
          -1;  // -1 means we don't have the high byte yet (0-15 means we do)
      fp_checksum = 0;
...
// For UDB Logo instructions, bytes should be passed in using the following
// format
...
// XX   byte:   checksum should equal the sum of the 10 bytes before the *, mod
// 256
//
// For example: "L0201000005*E8" runs:
// the DO command(02) for subroutine 01 with fly and param off(00) and an
// argument of 0005
...
// *                    done with command data
// XX       byte:       checksum should equal the sum of the 44 bytes before the
// *, mod 256
//
...
void serial_output(char* format, ...) {
  ...
  if (remaining > 1) {
    int16_t wrote = vsnprintf((char*)(&serial_buffer[start_index]),
...
void serial_output_8hz(void) {
  if (udb_heartbeat_counter % (HEARTBEAT_HZ / HEARTBEAT_UDB_TELEMETRY) == 0) {
    ...
    //#else
    //            if (tow.WW > 0) tow.WW +=
    // (int16_t)(1000/HEARTBEAT_UDB_TELEMETRY);
    //#endif
...
//    					serial_output("p%ii%i:",i,pwIn_save[i]);
    ...
    serial_output("t1%i;t2%i;t3%i;", tele_throttle1 - throttle_offset,
                  ...
                  tele_throttle3 - throttle_offset);
    serial_output("mt%i;th%i;", tele_mean_throttle - throttle_offset,
                  throttle_hover_control);
...
//                earth_yaw, tofinish_line_factor10,
// 100*(int16_t)(control_position_hold));
//
//            //GPS X-Y positioning : pitch control
//            serial_output("hp%i;tp%i;pa%i;pv%i;pc%i;",
//                hovering_pitch_order, target_pitch, -rmat[7], pitch_v_target,
// pitch_hover_corr);
//
//            //GPS X-Y positioning : roll control
//            serial_output("hr%i;tr%i;ra%i;rv%i;rc%i;",
//                hovering_roll_order, target_roll, rmat[6], roll_v_target,
// roll_hover_corr);
//
#if (USE_LIDAR == 1)
    serial_output("ld%i;lv%i;", lidar_distance,
                  100 * udb_flags._.lidar_height_valid);
#endif
#if (USE_SONAR == 1)
    serial_output("sd%i;sv%i;", sonar_distance,
                  100 * udb_flags._.sonar_height_valid);
#endif

#if (BAROMETER_ALTITUDE == 1 && SILSIM != 1)
    serial_output("ba%li;", get_barometer_altitude());
#endif
    serial_output("end;\r\n");
  }
}
