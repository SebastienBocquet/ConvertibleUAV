...
#define CALIB_PAUSE \
  21  // wait for 10.5 seconds of runs through the state machine
#define STANDBY_PAUSE \
  48  // pause for 24 seconds of runs through the state machine
...
//	Examine the state of the radio and GPS and supervisory channel to decide
...
void udb_background_callback_periodic(void) {
  if (counter++ >= 20)  // 2Hz
  {
    ...
    integral_pid_term_switch_check_set();
// Update the nav capable flag. If the GPS has a lock, gps_data_age will be
// small.
// For now, nav_capable will always be 0 when the Airframe type is
// AIRFRAME_HELI.
...
//	Calibrate state is used to wait for the filters to settle before
//recording A/D offsets.
...
#if (FIXED_TRIMPOINT != \
     1)  // Do not alter trims from preset when they are fixed
#if (USE_NV_MEMORY == 1)
...
//	Manual state is used for direct pass-through control from radio to
//servos.
...
static void ent_stabilizedS(void) {
  ...
  flags._.altitude_hold_pitch = (ALTITUDEHOLD_STABILIZED == AH_FULL ||
...
//	Come home is commanded by the mode switch channel (defaults to channel
//4).
static void ent_waypointS(void) {
  ...
  flags._.altitude_hold_pitch = (ALTITUDEHOLD_WAYPOINT == AH_FULL ||
                                 ALTITUDEHOLD_WAYPOINT == AH_PITCH_ONLY);

  if (!(FAILSAFE_TYPE == FAILSAFE_MAIN_FLIGHTPLAN && stateS == &returnS)) {
    init_flightplan(
        0);  // Only reset non-rtl waypoints if not already following waypoints
  }
...
//	Come home state, entered when the radio signal is lost, and gps is
//locked.
static void ent_returnS(void) {
  DPRINT("ent_returnS\r\n");
...
static void calibrateS(void) {
...
#endif
  {
    ...
    calib_timer--;
    if (calib_timer <= 0) ent_acquiringS();
  } else {
    //		DPRINT("calibrateS()\r\n");
    ent_calibrateS();
  }
...
static void stabilizedS(void) {
  if (udb_flags._.radio_on) {
    ...
    if (dcm_flags._.nav_capable) ent_returnS();
  }
}
