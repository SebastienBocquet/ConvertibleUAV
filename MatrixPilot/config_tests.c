...
#if (THROTTLE_HOVER_INPUT_CHANNEL > NUM_INPUTS)
#error("THROTTLE_HOVER_INPUT_CHANNEL > NUM_INPUTS.")
#endif
...
#if (FLAP_INPUT_CHANNEL > NUM_INPUTS)
#error("FLAP_INPUT_CHANNEL > NUM_INPUTS.")
#endif
...
#if (USE_SONAR == 1 && USE_SONAR_ON_PWM_INPUT_7 != 1 && \
     ANALOG_SONAR_INPUT_CHANNEL == 0)
...
#error("Must use NV memory with flexifunction mixing on UDB4+ only")
     ...
     (SERIAL_OUTPUT_FORMAT != SERIAL_MAVLINK))
