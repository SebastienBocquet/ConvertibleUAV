...
// Variables required for mavlink.  Used in AltitudeCntrlVariable and
// airspeedCntrl
...
int32_t excess_energy_height(void)  // computes (1/2gravity)*(actual_speed^2 -
                                    // desired_speed^2)
{
  ...
  forward_ground_speed.WW = ((__builtin_mulss(-IMUvelocityx._.W1, rmat[1]) +
                              __builtin_mulss(IMUvelocityy._.W1, rmat[4]))
                             << 2);
...
int32_t excess_energy_height(void) { return 0; }
...
void set_throttle_hover_control(int16_t throttle) {
  ...
  if (flags._.altitude_hold_throttle || flags._.altitude_hold_pitch ||
    ...
    int16_t temp = throttleIn +
                   REVERSE_IF_NEEDED(THROTTLE_HOVER_CHANNEL_REVERSED, throttle);

    if (THROTTLE_HOVER_CHANNEL_REVERSED) {
      if (temp > udb_pwTrim[THROTTLE_HOVER_INPUT_CHANNEL])
        throttle = throttleIn - udb_pwTrim[THROTTLE_HOVER_INPUT_CHANNEL];
    } else {
      if (temp < udb_pwTrim[THROTTLE_HOVER_INPUT_CHANNEL])
        throttle = udb_pwTrim[THROTTLE_HOVER_INPUT_CHANNEL] - throttleIn;
    }
...
// ent_stabilizedS().
      ...
      desiredHeight =
          ((__builtin_mulss((int16_t)(HEIGHTTHROTTLEGAIN),
    ...
    if (throttleInOffset < (int16_t)(DEADBAND) && udb_flags._.radio_on) {
      ...
      if (heightError._.W0 < (-(int16_t)(HEIGHT_MARGIN * 8.0))) {
        ...
        throttleAccum.WW = (int16_t)(MAXTHROTTLE) +
                           (__builtin_mulss((int16_t)(THROTTLEHEIGHTGAIN),
                                            (-heightError._.W0 -
                                             (int16_t)(HEIGHT_MARGIN * 8.0))) >>
                            3);
        if (throttleAccum.WW > (int16_t)(MAXTHROTTLE))
          throttleAccum.WW = (int16_t)(MAXTHROTTLE);
      }
      ...
      if (heightError._.W0 < (-(int16_t)(HEIGHT_MARGIN * 8.0))) {
        ...
        pitchAccum.WW = __builtin_mulss((int16_t)(PITCHHEIGHTGAIN),
                                        -heightError._.W0 -
                                            (int16_t)(HEIGHT_MARGIN * 8.0)) >>
                        3;
