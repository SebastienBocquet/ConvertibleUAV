...
int16_t compute_tx_linear_control(int16_t tx_order, int16_t order_min,
                                  int16_t order_max, int16_t reverse) {
...
// Mix yaw control and waggle into rudder
  ...
  udb_pwOut[AILERON_OUTPUT_CHANNEL] = udb_servo_pulsesat(
      temp + REVERSE_IF_NEEDED(FLAP_CHANNEL_REVERSED, flap_control));
...
// Mix pitch_control and yaw_control into both elevator and rudder
  ...
  udb_pwOut[AILERON_SECONDARY_OUTPUT_CHANNEL] =
      3000 + REVERSE_IF_NEEDED(AILERON_SECONDARY_CHANNEL_REVERSED,
                               udb_pwOut[AILERON_OUTPUT_CHANNEL] - 3000);

  temp = pwManual[ELEVATOR_INPUT_CHANNEL] +
         REVERSE_IF_NEEDED(ELEVATOR_CHANNEL_REVERSED,
                           pitch_control + vtail_yaw_control);
  udb_pwOut[ELEVATOR_OUTPUT_CHANNEL] = udb_servo_pulsesat(temp);
...
// Mix rudder_control into  rudder
  ...
  temp = pwManual[AILERON_INPUT_CHANNEL] +
         REVERSE_IF_NEEDED(AILERON_CHANNEL_REVERSED,
                           -delta_roll_control + pitch_control - waggle);
  udb_pwOut[AILERON_OUTPUT_CHANNEL] = udb_servo_pulsesat(temp);
...
// Ignore waggle for now
  ...
  temp = pwManual
      [RUDDER_INPUT_CHANNEL] /*+ REVERSE_IF_NEEDED(RUDDER_CHANNEL_REVERSED, yaw_control)*/;
  udb_pwOut[RUDDER_OUTPUT_CHANNEL] = udb_servo_pulsesat(temp);
  ...
  udb_pwOut[PASSTHROUGH_C_OUTPUT_CHANNEL] =
      udb_servo_pulsesat(pwManual[PASSTHROUGH_C_INPUT_CHANNEL]);
  udb_pwOut[PASSTHROUGH_D_OUTPUT_CHANNEL] =
...
void cameraServoMix(void) {
  ...
  temp =
      (pwManual[CAMERA_YAW_INPUT_CHANNEL] - 3000) +
