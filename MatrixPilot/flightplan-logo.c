...
enum {
  LOGO_VAL_ZERO = 0,
  // XX_INPUT_CHANNEL // leave room for input channels: 1 - NUM_INPUTS (up to
  // 15)
  DIST_TO_HOME = 16,
  DIST_TO_GOAL,
  ALT,
...
#define _RT(x, pr)   \
  { 4, 0, pr, 0, x } \
  ,
#define _SET_ANGLE(x, pr) \
  { 4, 0, pr, 1, x }      \
  ,
#define _USE_CURRENT_ANGLE \
  { 4, 0, 0, 2, 0 }        \
  ,
#define _USE_ANGLE_TO_GOAL \
  { 4, 0, 0, 3, 0 }        \
  ,

#define _MV_X(x, fl, pr) \
  { 5, fl, pr, 0, x }    \
  ,
#define _SET_X(x, fl, pr) \
  { 5, fl, pr, 1, x }     \
  ,
#define _MV_Y(y, fl, pr) \
  { 5, fl, pr, 2, y }    \
  ,
#define _SET_Y(y, fl, pr) \
  { 5, fl, pr, 3, y }     \
  ,
#define _MV_Z(z, fl, pr) \
  { 5, fl, pr, 4, z }    \
  ,
#define _SET_Z(z, fl, pr) \
  { 5, fl, pr, 5, z }     \
  ,
#define _USE_CURRENT_POS(fl) \
  { 5, fl, 0, 6, 0 }         \
  ,
#define _HOME(fl)    \
  { 5, fl, 0, 7, 0 } \
...
#define _SET_ABS_VAL_HIGH(x) \
  { 5, 0, 0, 8, x }          \
  ,  // Set the high and then low words for X and
#define _SET_ABS_X_LOW(x) \
  { 5, 0, 0, 9, x }       \
  ,  // then Y, as 4 consecutive instructions.
#define _SET_ABS_Y_LOW(y, fl) \
  { 5, fl, 0, 10, y }         \
  ,  // (as VAL_HIGH, X_LOW, VAL_HIGH, Y_LOW)
...
#define _SPEED_INCREASE(s, pr) \
  { 11, 0, pr, 0, s }          \
  ,
#define _SET_SPEED(s, pr) \
  { 11, 0, pr, 1, s }     \
  ,

#define _SET_INTERRUPT(fn) \
  { 12, 0, 0, 1, fn }      \
  ,
#define _CLEAR_INTERRUPT \
  { 12, 0, 0, 0, 0 }     \
  ,

#define _LOAD_TO_PARAM(val) \
  { 13, 0, 0, val, 0 }      \
  ,

#define _IF_EQ(val, x, pr) \
  { 14, 0, pr, val, x }    \
  ,
#define _IF_NE(val, x, pr) \
  { 15, 0, pr, val, x }    \
  ,
#define _IF_GT(val, x, pr) \
  { 16, 0, pr, val, x }    \
  ,
#define _IF_LT(val, x, pr) \
  { 17, 0, pr, val, x }    \
  ,
#define _IF_GE(val, x, pr) \
  { 18, 0, pr, val, x }    \
  ,
#define _IF_LE(val, x, pr) \
  { 19, 0, pr, val, x }    \
  ,
...
#define SET_ABS_POS(x, y)                                 \
  _SET_ABS_VAL_HIGH((((uint32_t)(x)) >> 16) & 0xFFFF)     \
      _SET_ABS_X_LOW(((uint32_t)(x)) & 0xFFFF)            \
      _SET_ABS_VAL_HIGH((((uint32_t)(y)) >> 16) & 0xFFFF) \
      _SET_ABS_Y_LOW(((uint32_t)(y)) & 0xFFFF, 1)
#define HOME _HOME(1)

#include "flightplan-logo.h"

#define NUM_INSTRUCTIONS \
  ((sizeof instructions) / sizeof(struct logoInstructionDef))
#define NUM_RTL_INSTRUCTIONS \
  ((sizeof rtlInstructions) / sizeof(struct logoInstructionDef))
int16_t instructionIndex = 0;
int16_t waypointIndex = 0;  // used for telemetry
int16_t absoluteHighWord = 0;
union longww absoluteXLong;

struct logoInstructionDef *currentInstructionSet =
    (struct logoInstructionDef *)instructions;
int16_t numInstructionsInCurrentSet = NUM_INSTRUCTIONS;

// If we've processed this many instructions without commanding the plane to
// fly,
...
int16_t interruptIndex =
    0;  // intruction index of the beginning of the interrupt function
int8_t interruptStackBase = 0;  // stack depth when entering interrupt (clear
                                // interrupt when dropping below this depth)
...
struct logoStackFrame {
  ...
  int16_t returnInstructionIndex : 14;  // instructionIndex before the first
                                        // instruction of the subroutine (a TO
                                        // or REPEAT line, or -1 for MAIN)
  int16_t arg : 16;
};
...
#define LOGO_MAIN \
  0  // Allows for DO(LOGO_MAIN) or EXEC(LOGO_MAIN) to start at the top
...
void init_flightplan(int16_t flightplanNum) {
  ...
  logoStack[logoStackIndex].returnInstructionIndex =
      -1;  // When starting over, begin on instruction 0
...
void run_flightplan(void) {
  ...
  if (desired_behavior._.altitude) {
    if (abs(IMUheight - goal.height) <
        ((int16_t)HEIGHT_MARGIN))  // reached altitude goal
    {
      process_instructions();
    }
  } else {
...
int16_t find_start_of_subroutine(uint8_t subcmd) {
  if (subcmd == 0)
    return -1;  // subcmd 0 is reserved to always mean the start of the logo
                // program

  int16_t i;
  for (i = 0; i < numInstructionsInCurrentSet; i++) {
    if (currentInstructionSet[i].cmd == 1 &&
        currentInstructionSet[i].subcmd == 2 &&
        currentInstructionSet[i].arg == subcmd) {
      return i;
    }
  }
  return -1;
...
// When an IF condition was true, and we ran the block, and reach an ELSE, skips
// to the END
uint16_t find_end_of_current_if_block(void) {
  ...
  for (i = instructionIndex + 1; i < numInstructionsInCurrentSet; i++) {
    if (currentInstructionSet[i].cmd == 1 &&
        currentInstructionSet[i].subcmd == 0)
      nestedDepth++;  // into a REPEAT
    else if (currentInstructionSet[i].cmd >= 14 &&
             currentInstructionSet[i].cmd <= 19)
      nestedDepth++;  // into an IF
    else if (nestedDepth > 0 && currentInstructionSet[i].cmd == 1 &&
             currentInstructionSet[i].subcmd == 1)
      nestedDepth--;  // nested END
    else if (nestedDepth == 0 && currentInstructionSet[i].cmd == 1 &&
             (currentInstructionSet[i].subcmd == 1 ||
              currentInstructionSet[i].subcmd == 3)) {
...
// Referencing PARAM in a LOGO program uses the PARAM from the current
// subroutine frame, even if
// we're also nested deeper inside of IF or REPEAT frames.  This finds the
...
int16_t logo_value_for_identifier(uint8_t ident) {
  ...
  switch (ident) {
    case DIST_TO_HOME:  // in m
      return sqrt_long(IMUlocationx._.W1 * (int32_t)IMUlocationx._.W1 +
                       IMUlocationy._.W1 * (int32_t)IMUlocationy._.W1);
    ...
    case REL_ANGLE_TO_HOME:  // in degrees. -180-179 (0=heading directly towards
    ...
    case REL_ANGLE_TO_GOAL:  // in degrees. -180-179 (0=heading directly towards
                             // goal. clockwise offset is positive)
    {
      int16_t angle = get_current_angle() -
                      get_angle_to_point(IMUlocationx._.W1, IMUlocationy._.W1);
      if (angle < -180) angle += 360;
      if (angle >= 180) angle -= 360;
      return angle;
    }
    ...
    case WIND_SPEED:  // in cm/s
      return sqrt_long(estimatedWind[0] * (int32_t)estimatedWind[0] +
                       estimatedWind[1] * (int32_t)estimatedWind[1]);
  ...
  switch (instr.cmd) {
          ...
          if (logoStackIndex > 0) {
            if (logoStack[logoStackIndex].frameType == LOGO_FRAME_TYPE_REPEAT) {
              ...
              if (logoStack[logoStackIndex].arg > 1 ||
                  logoStack[logoStackIndex].arg == -1) {
                if (logoStack[logoStackIndex].arg != -1) {
            ...
            } else if (logoStack[logoStackIndex].frameType ==
                       LOGO_FRAME_TYPE_SUBROUTINE) {
              // END SUBROUTINE
              instructionIndex =
                  logoStack[logoStackIndex].returnInstructionIndex;
              logoStackIndex--;
              if (logoStackIndex < interruptStackBase) {
                ...
                instructionsProcessed =
                    MAX_INSTRUCTIONS_PER_CYCLE;  // stop processing instructions
                                                 // after finishing interrupt
              }
            } else if (logoStack[logoStackIndex].frameType ==
                       LOGO_FRAME_TYPE_IF) {
    ...
    case 3:  // Forward/Back
      switch (instr.subcmd) {
        ...
        {
          int16_t cangle =
              turtleAngles[currentTurtle];  // 0-359 (clockwise, 0=North)
          int8_t b_angle =
              (cangle * 182 + 128) >> 8;  // 0-255 (clockwise, 0=North)
          b_angle = -b_angle - 64;        // 0-255 (ccw, 0=East)

          turtleLocations[currentTurtle].x.WW +=
              (__builtin_mulss(-cosine(b_angle), instr.arg) << 2);
          turtleLocations[currentTurtle].y.WW +=
              (__builtin_mulss(-sine(b_angle), instr.arg) << 2);
        } break;
      }
      break;
    ...
    case 19: {
      ...
      if (instr.cmd == 14 && val == instr.arg)
        condTrue = true;  // IF_EQ
      else if (instr.cmd == 15 && val != instr.arg)
        condTrue = true;  // IF_NE
      else if (instr.cmd == 16 && val > instr.arg)
        condTrue = true;  // IF_GT
      else if (instr.cmd == 17 && val < instr.arg)
        condTrue = true;  // IF_LT
      else if (instr.cmd == 18 && val >= instr.arg)
        condTrue = true;  // IF_GE
      else if (instr.cmd == 19 && val <= instr.arg)
        condTrue = true;  // IF_LE
      ...
      if (condTrue) {
        ...
        instructionIndex = find_end_of_current_if_block();
        if (currentInstructionSet[instructionIndex].subcmd ==
            3)  // is entering an ELSE block
        {
          if (logoStackIndex < LOGO_STACK_DEPTH - 1) {
            logoStackIndex++;
            logoStack[logoStackIndex].frameType = LOGO_FRAME_TYPE_IF;
...
void process_instructions(void) {
  ...
  while (1) {
    boolean do_fly =
        process_one_instruction(currentInstructionSet[instructionIndex]);

    instructionsProcessed++;
    instructionIndex++;
    if (instructionIndex >= numInstructionsInCurrentSet) instructionIndex = 0;

    if (do_fly && penState == 0 && currentTurtle == PLANE) break;
...
void flightplan_live_begin(void) {
  if (logo_inject_pos == LOGO_INJECT_READY) return;
  logo_inject_pos = 0;
}
