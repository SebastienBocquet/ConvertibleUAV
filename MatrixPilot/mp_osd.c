...
static const uint8_t heading_strings[16][4] = {{0x8F, 0x00, 0x00, 0xFF},  // E
...
static void osd_update_horizon(void) {
  ...
  int32_t earth_roll =
      rect_to_polar(&matrix_accum);  // binary angle (0 - 256 = 360 degrees)
  earth_roll = (-earth_roll * BYTECIR_TO_DEGREE) >>
               16;  // switch polarity, convert to -180 - 180 degrees
  ...
  int32_t earth_pitch =
      rect_to_polar(&matrix_accum);  // binary angle (0 - 256 = 360 degrees)
  earth_pitch = (-earth_pitch * BYTECIR_TO_DEGREE) >>
                16;  // switch polarity, convert to -180 - 180 degrees
  ...
  for (i = -OSD_HORIZON_WIDTH; i < OSD_HORIZON_WIDTH; i++) {
    ...
    if (lastHeight != 0 || OSD_SHOW_CENTER_DOT != 1 || (i != -1 && i != 0)) {
      if (height != lastHeight && lastHeight >= -OSD_SPACING &&
...
static void osd_setup_screen(void) {
  ...
  osd_spi_write_vertical_string_at_location(OSD_LOC_CALLSIGN_VERT,
                                            callsign);  // callsign
#endif
}
...
static void osd_update_values(void) {
  switch (osd_phase) {
    case 0: {
      ...
      osd_spi_write_number(IMUlocationz._.W1, 0, 0, NUM_FLAG_SIGNED, 0,
                           0);  // Altitude
#endif
      ...
      osd_spi_write_number(IMUvelocityz._.W1, 0, 0, NUM_FLAG_SIGNED, 0,
                           0);  // Variometer
#endif
    ...
    case 1: {
      ...
      osd_spi_write_string(
          heading_strings[((uint8_t)(earth_yaw + 8)) >> 4]);  // heading
#endif
      ...
      if (dist_to_goal != 0) {
        ...
        verticalAngle = rect_to_polar(
            &componentsToPlane);  // binary angle (0 - 256 = 360 degrees)
        verticalAngle = (verticalAngle * BYTECIR_TO_DEGREE) >>
                        16;  // switch polarity, convert to -180 - 180 degrees
      }
      osd_spi_write_location(OSD_LOC_VERTICAL_ANGLE_HOME);
      osd_spi_write_number(verticalAngle, 0, 0, NUM_FLAG_SIGNED, 0,
                           0x4D);  // Footer: Degree symbol
#endif

#if (OSD_LOC_ROLL_RATE != OSD_LOC_DISABLED)
      osd_spi_write_location(OSD_LOC_ROLL_RATE);
      osd_spi_write_number(abs(omegagyro[1]) / DEGPERSEC, 3, 0, 0, 0,
                           0);  // roll rate in degrees/sec/sec
#endif

#if (OSD_LOC_PITCH_RATE != OSD_LOC_DISABLED)
      osd_spi_write_location(OSD_LOC_PITCH_RATE);
      osd_spi_write_number(abs(omegagyro[0]) / DEGPERSEC, 3, 0, 0, 0,
                           0);  // pitch rate in degrees/sec/sec
#endif

#if (OSD_LOC_YAW_RATE != OSD_LOC_DISABLED)
      osd_spi_write_location(OSD_LOC_YAW_RATE);
      osd_spi_write_number(abs(omegagyro[2]) / DEGPERSEC, 3, 0, 0, 0,
                           0);  // yaw rate in degrees/sec/sec
#endif
      ...
      osd_spi_write_number(battery_current._.W1, 3, 1, 0, 0,
                           0xB4);  // tenths of Amps being used right now
#endif

#if (OSD_LOC_BATT_USED != OSD_LOC_DISABLED)
      osd_spi_write_location(OSD_LOC_BATT_USED);
      osd_spi_write_number(battery_mAh_used._.W1, 4, 0, 0, 0,
                           0xB7);  // mAh used so far
#endif
      ...
      osd_spi_write_number(battery_voltage._.W1, 3, 1, 0, 0,
                           0xA0);  // tenths of Volts
#endif
      ...
      osd_spi_write_number(rc_signal_strength, 3, 0, 0, 0,
                           0xB3);  // RC Receiver signal strength as 0-100%
#endif
    ...
    case 3: {
      ...
      osd_spi_write_number(air_speed_3DIMU / 100, 3, 0, 0, 0,
                           0);  // speed in m/s
#endif

#if (OSD_LOC_AIR_SPEED_MI_HR != OSD_LOC_DISABLED)
      osd_spi_write_location(OSD_LOC_AIR_SPEED_MI_HR);
      osd_spi_write_number(air_speed_3DIMU / 45, 3, 0, 0, 0,
                           0);  // speed in mi/hr
#endif

#if (OSD_LOC_AIR_SPEED_KM_HR != OSD_LOC_DISABLED)
      osd_spi_write_location(OSD_LOC_AIR_SPEED_KM_HR);
      osd_spi_write_number(air_speed_3DIMU / 28, 3, 0, 0, 0,
                           0);  // speed in km/hr
#endif

#if (OSD_LOC_GROUND_SPEED_M_S != OSD_LOC_DISABLED ||   \
     OSD_LOC_GROUND_SPEED_MI_HR != OSD_LOC_DISABLED || \
     OSD_LOC_GROUND_SPEED_KM_HR != OSD_LOC_DISABLED)
      ...
      osd_spi_write_number(ground_speed_3DIMU / 100, 3, 0, 0, 0,
                           0);  // speed in m/s
#endif

#if (OSD_LOC_GROUND_SPEED_MI_HR != OSD_LOC_DISABLED)
      osd_spi_write_location(OSD_LOC_GROUND_SPEED_MI_HR);
      osd_spi_write_number(ground_speed_3DIMU / 45, 3, 0, 0, 0,
                           0);  // speed in mi/hr
#endif

#if (OSD_LOC_GROUND_SPEED_KM_HR != OSD_LOC_DISABLED)
      osd_spi_write_location(OSD_LOC_GROUND_SPEED_KM_HR);
      osd_spi_write_number(ground_speed_3DIMU / 28, 3, 0, 0, 0,
                           0);  // speed in km/hr
#endif
      ...
      osd_spi_write_number(
          (ZACCEL_VALUE - gravity_z._.W1) / (100 * ACCELSCALE), 3, 0,
          NUM_FLAG_SIGNED, 0,
          0);  // vertical acceleration rate in units of m/sec/sec
#endif

#if (OSD_LOC_VERTICAL_WIND_SPEED != OSD_LOC_DISABLED)
      osd_spi_write_location(OSD_LOC_VERTICAL_WIND_SPEED);
      osd_spi_write_number(estimatedWind[2] / 10, 4, 1, NUM_FLAG_SIGNED, 0,
                           0);  // vertical wind speed in m/s
#endif

#if (OSD_LOC_TOTAL_ENERGY != OSD_LOC_DISABLED)
      osd_spi_write_location(OSD_LOC_TOTAL_ENERGY);
      osd_spi_write_number(total_energy, 4, 0, NUM_FLAG_SIGNED, 0,
                           0);  // total energy in meters above the origin
#endif

#if (OSD_AUTO_HIDE_GPS == 1)
      boolean showGPS =
          (IMUlocationz._.W1 < 20 || ground_velocity_magnitudeXY < 150);
#else
      boolean showGPS = 1;
#endif
      ...
      if (showGPS) {
        osd_spi_write_number(svs, 0, 0, 0, 0xEB, 0);  // Num satelites locked,
                                                      // with SatDish icon
                                                      // header
      } else {
      ...
      if (showGPS) {
        osd_spi_write_number(labs(lat_gps.WW / 10), 8, 6, 0, 0,
                             (lat_gps.WW >= 0) ? 0x98 : 0x9D);  // Footer: N/S
      } else {
      ...
      if (showGPS) {
        osd_spi_write_number(labs(long_gps.WW / 10), 9, 6, 0, 0,
                             (long_gps.WW >= 0) ? 0x8F : 0xA1);  // Footer: E/W
      } else {
...
void osd_run_step(void) {
  boolean osd_on = (OSD_MODE_SWITCH_INPUT_CHANNEL == CHANNEL_UNUSED ||
                    udb_pwIn[OSD_MODE_SWITCH_INPUT_CHANNEL] >= 3000 ||
  ...
  if (countdown == 61) {
    osd_spi_write_byte(0xFF);  // Terminate sending a string, in case that was
                               // happening (Prep for reset)
