...
int16_t airspeed_pitch_adjust_rate =
    (AIRSPEED_PITCH_ADJ_RATE * (RMAX / (57.3 * 40.0)));
...
extern int32_t calc_airspeed_int_error(int16_t aspdError,
                                       int32_t aspd_integral);
...
void airspeedCntrl(void) {
  ...
  airspeed_error_integral.WW =
      calc_airspeed_int_error(airspeedError, airspeed_error_integral.WW);
}
...
int16_t calc_groundspeed(void)  // computes (1/2gravity)*(actual_speed^2 -
                                // desired_speed^2)
{
  int32_t gndspd2;
...
int16_t calc_target_airspeed(int16_t desiredSpd) {
  ...
  if (target > maximum_airspeed) target = maximum_airspeed;

  if (target < minimum_airspeed) target = minimum_airspeed;
