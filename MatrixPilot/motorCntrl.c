...
#define MANUAL_DEADBAND \
  200  // amount of throttle before fly-by-wire controls engage
...
// coefficients on manual throttle to determine engine throttle ensuring equiibrium of the tricopter
#define K_AB  (K_A / K_B)
#define K_BA  (K_B / K_A)
...
void motorCntrl(const uint16_t tilt_kp, const uint16_t tilt_ki,
                const uint16_t tilt_rate_kp, const uint16_t tilt_rate_kd,
  ...
  int16_t yaw_control_weight = compute_tx_linear_control(
      pwManual[INPUT_CHANNEL_AUX2], 0, RMAX, yaw_control_weight_reverse);
  ...
  if (!dcm_flags._.calib_finished) {
    ...
    MatrixMultiply(3, 3, 3, orientation_error_matrix,
                   target_orientation_transposed, rmat);
    ...
    roll_error = rmat[6] - (-commanded_roll_body_frame +
                            roll_control * commanded_tilt_gain);
    pitch_error = -rmat[7] - (-commanded_pitch_body_frame +
                              pitch_control * commanded_tilt_gain);
    ...
    if ((flags._.integral_pid_term) && (current_orientation == F_HOVER)) {
      roll_quad_error_integral.WW +=
          ((__builtin_mulus((uint16_t)(32.0 * tilt_ki / 40.), roll_error)) >>
           5);
      ...
      pitch_quad_error_integral.WW +=
          ((__builtin_mulus((uint16_t)(32.0 * tilt_ki / 40.), pitch_error)) >>
           5);
      ...
      yaw_quad_error_integral.WW +=
          ((__builtin_mulus((uint16_t)(32.0 * yaw_ki / 40.), yaw_error)) >> 5);
    ...
    roll_intgrl = limit_value(roll_quad_error_integral._.W1 << 2,
                              -(int16_t)(TILT_ERROR_INTEGRAL_LIMIT),
                              (int16_t)(TILT_ERROR_INTEGRAL_LIMIT));
    ...
    roll_rate_error_delta_filt = exponential_filter(
        roll_rate_error_delta, &roll_rate_error_delta_filt_flt,
    ...
    long_accum.WW = __builtin_mulus(tilt_rate_kd, roll_rate_error_delta_filt)
                    << 2;
    roll_quad_control -= long_accum._.W1;
    ...
    pitch_intgrl = limit_value(pitch_quad_error_integral._.W1 << 2,
                               -(int16_t)(TILT_ERROR_INTEGRAL_LIMIT),
                               (int16_t)(TILT_ERROR_INTEGRAL_LIMIT));
    ...
    // exponential_filter(pitch_rate, &pitch_rate_filtered_flt, (float)(80),
    ...
    pitch_rate_error_delta_filt = exponential_filter(
        pitch_rate_error_delta, &pitch_rate_error_delta_filt_flt,
        (float)(TILT_RATE_DELTA_FILTER));
    ...
    long_accum.WW = __builtin_mulus(tilt_rate_kd, pitch_rate_error_delta_filt)
                    << 2;
    pitch_quad_control -= long_accum._.W1;
