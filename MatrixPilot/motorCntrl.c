...
//	union longww accum ; // debugging temporary
	...
	else if(canStabilizeHover() && current_orientation == F_HOVER)
	{
...
//		}
	...
	{
		motor_A = motor_B = motor_C = motor_D = pwManual[THROTTLE_HOVER_INPUT_CHANNEL];
		udb_pwOut[MOTOR_A_OUTPUT_CHANNEL] = motor_A ;
		udb_pwOut[MOTOR_B_OUTPUT_CHANNEL] = motor_B ;
		udb_pwOut[MOTOR_C_OUTPUT_CHANNEL] = motor_C ;
		udb_pwOut[MOTOR_D_OUTPUT_CHANNEL] = motor_D ;
	}
}
