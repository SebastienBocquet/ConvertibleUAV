...
// Enable or disable the whole OSD system by setting the value of USE_OSD in
// options.h

// OSD_VIDEO_FORMAT can be set to either OSD_NTSC, or OSD_PAL
#define OSD_VIDEO_FORMAT OSD_NTSC

// The callsign is written using characters from the OSD Font file.  (See
// Tools/FlashOSD/.)
// This has to end with 0xFF.
// Adjust the placement using OSD_LOC_CALLSIGN_HORIZ and OSD_LOC_CALLSIGN_VERT.
#define OSD_CALL_SIGN \
  { 0x95, 0x81, 0x82, 0x83, 0x84, 0x85, 0xFF }  // K12345
...
#define OSD_AUTO_HIDE_GPS \
  1  // Only show Lat, Long, and Num Sats while slow and low.

// OSD Element Locations
// Set each one to OSD_LOC_DISABLED or OSD_LOC(row, col) from (0, 0) to (12, 29)
// for NTSC or up to (15, 29) for PAL.
