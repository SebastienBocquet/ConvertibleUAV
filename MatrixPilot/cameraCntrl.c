...
//			(flags._.GPS_steering == 1 && flags._.pitch_feedback == 1))
#else
		if ((udb_pwIn[CAMERA_MODE_INPUT_CHANNEL] > CAMERA_MODE_THRESHOLD_LOW) && \
...
//#ifdef USE_DEBUG_IO
//            printf("camera_view %i, %i, %i\r\n", camera_view.x, camera_view.y, camera_view.z);
//#endif
			...
			camera_view.x =   0; //- sine(cam_yaw8);  //controls target roll angle of camera [-90° ; 90°] -> [-16384 ; 16384]
			camera_view.y =   0; //  cosine(cam_yaw8);  //controls target pitch angle of camera [-90° ; 90°] -> [-16384 ; 16384]
		...
		cam_yaw16 = rect_to_polar16(&matrix_accum); // Note matrix_accum.x is the left over result of yaw call to rect_to_polar16

		//Convert camera vector (which is in plane's coordinaet reference) to a roll angle.
