...
// servo_ratios are used to convert degrees of rotation into servo pulse code
// lengths
// This code is configured for the full throw of the servo to be achieved by a
// range of
// 2000 units being sent to udb_pwOut. (i.e. min deflection 2000, centered 3000,
// max deflection 4000)
#define PITCH_SERVO_HIGH_RATIO \
  ((2000.0 / ((CAM_PITCH_SERVO_THROW / 360.0) * 65536.0)) * 65536.0)
#define YAW_SERVO_HIGH_RATIO \
  ((2000.0 / ((CAM_YAW_SERVO_THROW / 360.0) * 65536.0)) * 65536.0)
...
const int16_t pitch_offset_centred_pwm =
    (CAM_PITCH_OFFSET_CENTRED * 65536.0 / 360.0) * PITCH_SERVO_RATIO;
const int16_t yaw_offset_centred_pwm =
    (CAM_YAW_OFFSET_CENTRED * 65536.0 / 360.0) * YAW_SERVO_RATIO;

const int16_t pitch_servo_pwm_max =
    ((CAM_PITCH_SERVO_MAX - CAM_PITCH_OFFSET_CENTRED) * 65536.0 / 360.0) *
    PITCH_SERVO_RATIO;
;
const int16_t pitch_servo_pwm_min =
    ((CAM_PITCH_SERVO_MIN - CAM_PITCH_OFFSET_CENTRED) * 65536.0 / 360.0) *
    PITCH_SERVO_RATIO;
const int16_t yaw_servo_pwm_max =
    ((CAM_YAW_SERVO_MAX - CAM_YAW_OFFSET_CENTRED) * 65536.0 / 360.0) *
    YAW_SERVO_RATIO;
const int16_t yaw_servo_pwm_min =
    ((CAM_YAW_SERVO_MIN - CAM_YAW_OFFSET_CENTRED) * 65536.0 / 360.0) *
    YAW_SERVO_RATIO;

struct relative3D view_location = {0, 20, 0};
struct relative3D camera_view = {0, 0, 0};

#if (CAM_TESTING_OVERIDE == 1)  // Used to test that Camera swings by correct
                                // angles when camera control gains.
#define CAM_TEST_TIMER \
  200  // e.g. value of 200 means 5 seconds (200 decremented 40 times / second
       // until zero).
...
#if (CAM_PITCH_TEST_GRANULARITY == \
     1)  // Used to test the smallest movement possible by pitch servo.
...
void cameraCntrl(void) {
  ...
  // int8_t cam_yaw8 = 0;        // An 8 bit version of cam_yaw to use with
  // sine(), cosine()

  struct relative2D matrix_accum = {
      0, 0};  // Temporary variable to keep intermediate results of functions.
  fractional cam_vector_ground[] = {0, 0, 0};  // Vector to camera target from
                                               // within ground coordinate
                                               // reference.
  fractional cam_vector_plane[] = {0, 0, 0};   // Vector to camera target from
                                               // within plane's coordinate
                                               // reference
...
// Stabilised Mode and GPS mode
#if (CAMERA_MODE_INPUT_CHANNEL == CHANNEL_UNUSED)
    if (flags._.pitch_feedback == 1)
//		if ((flags._.GPS_steering == 0 && flags._.pitch_feedback == 1)
//||
//			(flags._.GPS_steering == 1 && flags._.pitch_feedback ==
//1))
#else
    if ((udb_pwIn[CAMERA_MODE_INPUT_CHANNEL] > CAMERA_MODE_THRESHOLD_LOW) &&
...
#endif
    {
      ...
      // Against the ground reference, the camera has to stay at a constant
      // pitch to the horizontal
      // Against the plane reference, the camera has to keep at a constant yaw.
      // e.g. straight ahead.
      // Maths:
      // Calculate the Yaw angle of the Plane in the ground reference.
      // Use the desired pitch of the camera, to create a camera view vector
      ...
      //#ifdef USE_DEBUG_IO
      //            printf("camera_view %i, %i, %i\r\n", camera_view.x,
      // camera_view.y, camera_view.z);
      //#endif
      // camera pointing vertically downward
      // to be redefined with two angles (roll and pitch)
      // add same initialization of camera_view in manual mode
      // in GPS mode do not overwrite camera_view
      camera_view.x = 0;  //- sine(cam_yaw8);  //controls target roll angle of
                          //camera [-90° ; 90°] -> [-16384 ; 16384]
      camera_view.y = 0;  //  cosine(cam_yaw8);  //controls target pitch angle
                          // of camera [-90° ; 90°] -> [-16384 ; 16384]
    ...
    // and rotate into the plane's reference. This requires the use of the
    // inverse
    // of rmat which is also the transpose of the rmat matrix.
    // Then calculate each of the angles for yaw and pitch in the plane's
    // reference. (roll not implemented at this time).
    // Finally, convert camera angles in yaw and pitch to servo rotation angles.

    // Convert externally requested camera view into a structure of type
    // fractional
    // Convert from "UAV Devboard - Ground" convention to "Aviation Convention -
    // Ground"
    ...
    // It does not matter that the result of the following operation is not the
    // expected magnitude
    // because the code only uses the ratios of X,Y,Z relative to each other to
    // calculate angles.
    MatrixMultiply(3, 3, 1, cam_vector_plane, rmat_transpose,
                   cam_vector_ground);

// Convert camera vector which is now in plane's coordinate reference, to a Yaw
// angle with respect to front of plane.
// matrix_accum.x =   cam_vector_plane[0] ;
// matrix_accum.y =   cam_vector_plane[1] ;
// cam_yaw16 = rect_to_polar16(&matrix_accum) - 16384; // subtract 90 degrees so
// yaw measured in line with fuselage
    ...
    // Convert camera vector (which is in plane's coordinaet reference) to a
    // pitch angle.
    matrix_accum.x = cam_vector_plane[0];
    matrix_accum.y = cam_vector_plane[2];
    cam_yaw16 = rect_to_polar16(&matrix_accum);  // Note matrix_accum.x is the
                                                 // left over result of yaw call
                                                 // to rect_to_polar16

    // Convert camera vector (which is in plane's coordinaet reference) to a
    // roll angle.
    // override yaw angle by roll angle
    matrix_accum.x = cam_vector_plane[1];
    matrix_accum.y = cam_vector_plane[2];
    // sign_roll = cam_vector_plane[1] / sqrt(pow(cam_vector_plane[1] ,2));
    cam_pitch16 = rect_to_polar16(&matrix_accum);

#if (CAM_TESTING_OVERIDE == 1)
    cam_pitch16 = cam_testing_pitch_angle;
#endif

    // One day, insert special logic for when camera nearly pointing straight
    // down
    // to prevent large movements of camera on yaw for small changes in roll and
    // pitch.
    if (cam_pitch16 < -15000) {
      cam_pitch16 = 32767;
    }

// Calculate signal to send to pitch servo
#if (CAM_PITCH_TEST_GRANULARITY == 1)
    // Move camera by smallest pitch granular amount, continuously. (testing for
    // blurring of camera picture)
    if (counter_slow_down-- == 0) {
      if (pitch_servo_out-- < -500)
        pitch_servo_out = 500;  // A 22.5 degree movement either side of servo
                                // centre for a 90 degree servo
...
struct relative3D cam_inject;  // Camera view location received on the serial
                               // port
uint8_t cam_inject_pos = 0;

void camera_live_begin(void) { cam_inject_pos = 0; }
