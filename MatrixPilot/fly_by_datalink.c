...
int16_t get_fbdl_pwm(int16_t index) { return fbdl_pwm[index]; }

uint8_t get_fbdl_pos(void) { return fbdl_inject_pos; }

void fbdl_live_begin(void) {
  fbdl_inject_pos =
      1;  // The data parser never sees the first header value on UART
}
...
void fbdl_live_commit(void) { fbdl_live_commit_buf(fbdl_inject); }
