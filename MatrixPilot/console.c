...
void cmd_barom(void) {
  printf("Barometer temp %i, pres %u, alt %u, agl %u\r\n",
         get_barometer_temperature(), (uint16_t)get_barometer_pressure(),
...
void cmd_reg(void) {
  ...
  1 = The VBUS voltage is above VA_VBUS_VLD (as defined in the USB OTG
  Specification) on the A device
...
const cmds_t cmdslist[] = {{0, cmd_help, "help"},
                           {0, cmd_ver, "ver"},
                           {0, cmd_format, "format"},
                           {0, cmd_start, "start"},
                           {0, cmd_stop, "stop"},
