...
// called from telemetry module at interrrupt level to manage the log data
// buffers
...
static void log_write(char* str, int len) {
  //	unsigned char str_put_n_chars (FSFILE * handle, unsigned char n, char
  //c);
  if (fsp) {
...
// called from mainloop at background priority to write telemetry log data to
// log file
