...
#define THROTTLEHEIGHTGAIN                                                \
  (((ALT_HOLD_THROTTLE_MAX - ALT_HOLD_THROTTLE_MIN) * 2.0 * SERVORANGE) / \
   (HEIGHT_MARGIN * 2.0))
...
#define HEIGHTTHROTTLEGAIN                                    \
  ((1.5 * (HEIGHT_TARGET_MAX - HEIGHT_TARGET_MIN) * 1024.0) / \
   (SERVORANGE* SERVOSAT))
...
// Initialize to the value from options.h.  Allow updating this value from
// LOGO/MavLink/etc.
...
int32_t excess_energy_height(int16_t targetAspd,
                             int16_t acutalAirspeed)  // computes
    // (1/2gravity)*(actual_speed^2
    // - desired_speed^2)
{
  union longww accum;
  ...
  int32_t equivalent_energy_air_speed =
      -(__builtin_mulss(speedAccum, speedAccum));
...
void set_throttle_control(int16_t throttle) {
  ...
  if (flags._.altitude_hold_throttle || flags._.altitude_hold_pitch ||
    ...
    int16_t temp =
        throttleIn + REVERSE_IF_NEEDED(THROTTLE_CHANNEL_REVERSED, throttle);

    if (THROTTLE_CHANNEL_REVERSED) {
      if (temp > udb_pwTrim[THROTTLE_INPUT_CHANNEL])
        throttle = throttleIn - udb_pwTrim[THROTTLE_INPUT_CHANNEL];
    } else {
      if (temp < udb_pwTrim[THROTTLE_INPUT_CHANNEL])
        throttle = udb_pwTrim[THROTTLE_INPUT_CHANNEL] - throttleIn;
    }
...
void setTargetAltitude(int16_t targetAlt) { desiredHeight = targetAlt; }
...
void normalAltitudeCntrl(void) {
  ...
  temp.WW = __builtin_mulss((alt_hold_throttle_max - alt_hold_throttle_min),
                            2.0 * SERVORANGE);
  temp.WW <<= 2;
  if (temp._.W0 & 0x8000) temp._.W1++;
  temp._.W0 = temp._.W1;
  ...
  temp.WW =
      __builtin_mulss(alt_hold_pitch_max, (int16_t)((RMAX * 64.0) / 57.3));
  temp.WW <<= 10;
  if (temp._.W0 & 0x8000) temp._.W1++;
  pitch_at_max = temp._.W1;

  temp.WW =
      __builtin_mulss(alt_hold_pitch_min, (int16_t)((RMAX * 64.0) / 57.3));
  temp.WW <<= 10;
  if (temp._.W0 & 0x8000) temp._.W1++;
  pitch_at_min = temp._.W1;

  temp.WW =
      __builtin_mulss(alt_hold_pitch_high, (int16_t)((RMAX * 64.0) / 57.3));
  temp.WW <<= 10;
  if (temp._.W0 & 0x8000) temp._.W1++;
  ...
  temp.WW =
      __builtin_mulss((height_target_max - height_target_min), 1.5 * 1024.0);
  temp.WW <<= 2;
  ...
  speed_height = excess_energy_height(
      target_airspeed, airspeed);  // equivalent height of the airspeed

  if (udb_flags._.radio_on == 1) {
    throttleIn = udb_pwIn[THROTTLE_INPUT_CHANNEL];
    // keep the In and Trim throttle values within 2000-4000 to account for
    // Spektrum receivers using failsafe values below 2000.
    throttleInOffset = udb_servo_pulsesat(udb_pwIn[THROTTLE_INPUT_CHANNEL]) -
                       udb_servo_pulsesat(udb_pwTrim[THROTTLE_INPUT_CHANNEL]);
  } else {
    throttleIn = udb_pwTrim[THROTTLE_INPUT_CHANNEL];
    throttleInOffset = 0;
  }
  ...
  if (flags._.altitude_hold_throttle || flags._.altitude_hold_pitch) {
    ...
    if (flags._.GPS_steering) {
      if (desired_behavior._.takeoff || desired_behavior._.altitude) {
        ...
        desiredHeight =
            goal.fromHeight +
            (((goal.height - goal.fromHeight) * (int32_t)progress_to_goal) >>
             12);
      }
    } else {
#if (ALTITUDEHOLD_STABILIZED == AH_PITCH_ONLY)
// In stabilized mode using pitch-only altitude hold, use desiredHeight as
// set from the state machine upon entering stabilized mode in
// ent_stabilizedS().
#elif((ALTITUDEHOLD_STABILIZED == AH_FULL) || \
      (ALTITUDEHOLD_STABILIZED == AH_THROTTLE_ONLY))
      // In stabilized mode using full altitude hold, use the throttle stick
      // value to determine desiredHeight,
      desiredHeight =
          ((__builtin_mulss(height_throttle_gain,
                            throttleInOffset - ((int16_t)(DEADBAND)))) >>
           11) +
          height_target_min;
#endif
      if (desiredHeight < (int16_t)(height_target_min))
        desiredHeight = (int16_t)(height_target_min);
      if (desiredHeight > (int16_t)(height_target_max))
        desiredHeight = (int16_t)(height_target_max);
    }
    ...
    if (throttleInOffset < (int16_t)(DEADBAND) && udb_flags._.radio_on) {
      ...
      if (heightError._.W0 < -height_marginx8) {
        ...
        throttleAccum.WW =
            ...
            (__builtin_mulss(throttle_height_gain,
                             (-heightError._.W0 - height_marginx8)) >>
             3);
        if (throttleAccum.WW > (int16_t)(max_throttle))
          throttleAccum.WW = (int16_t)(max_throttle);
      }
      ...
      if (heightError._.W0 < -height_marginx8) {
        ...
        pitchAccum.WW = __builtin_mulss((int16_t)(pitch_height_gain),
                                        -heightError._.W0 - height_marginx8) >>
                        3;
    ...
    if (!flags._.altitude_hold_throttle) {
      ...
      throttleFiltered.WW += (((int32_t)(udb_pwTrim[THROTTLE_INPUT_CHANNEL] -
                                         throttleFiltered._.W1))
                              << THROTTLEFILTSHIFT);
      set_throttle_control(throttleFiltered._.W1 - throttleIn);
      filterManual = true;
    } else {
      // Servo reversing is handled in servoMix.c
      int16_t throttleOut = udb_servo_pulsesat(
          udb_pwTrim[THROTTLE_INPUT_CHANNEL] + throttleAccum.WW);
      throttleFiltered.WW += (((int32_t)(throttleOut - throttleFiltered._.W1))
                              << THROTTLEFILTSHIFT);
      set_throttle_control(throttleFiltered._.W1 - throttleIn);
      filterManual = true;
    }
...
void manualThrottle(int16_t throttleIn) {
  ...
  throttleFiltered.WW +=
      (((int32_t)(throttleIn - throttleFiltered._.W1)) << THROTTLEFILTSHIFT);

  if (filterManual) {
    // Continue to filter the throttle control value in manual mode to avoid
    // large, instant
    // changes to throttle value, which can burn out a brushed motor.  But after
    // fading over
    // to the new throttle value, stop applying the filter to the throttle out
    // to allow
    // faster control.
...
void hoverAltitudeCntrl(void) {
  int16_t throttle_control_pre;
  int16_t throttleIn = (udb_flags._.radio_on == 1)
                           ? udb_pwIn[THROTTLE_INPUT_CHANNEL]
