...
//#define CPU_LOAD_PERCENT  1678  // = ((65536 * 100) / ((32000000 / 2) / (16 *
//256)))
//#define CPU_LOAD_PERCENT  839   // = ((65536 * 100) / ((64000000 / 2) / (16 *
//256)))
...
#define HEARTBEAT_MAX \
  57600  // Evenly divisible by many common values: 2^8 * 3^2 * 5^2
...
#endif
  //	PR1 = 50000;            // 25 millisecond period at 16 Mz clock, tmr
  //prescale = 8
  PR1 = (FREQOSC / (TMR1_PRESCALE * CLK_PHASES)) /
        HEARTBEAT_HZ;  // period 1/HEARTBEAT_HZ
...
void __attribute__((__interrupt__, __no_auto_psv__)) _T1Interrupt(void) {
  ...
  // Start the sequential servo pulses at frequency OUTPUT_HZ (pins 1 to 4)
  if (udb_heartbeat_counter % (HEARTBEAT_HZ / ESC_HZ) == 0) {
    start_pwm_outputs();
    start_pwm_outputs_2();
  }

  // Start the sequential servo pulses at frequency OUTPUT_HZ (pins 4 to 8)
...
//	Executes whatever lower priority calculation needs to be done every
//heartbeat (default: 25 milliseconds)
