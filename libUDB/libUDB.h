...
// Call this function to trigger the udb_background_callback_triggered()
// function
...
// Set these values in your implementation of the
...
extern union udb_fbts_byte {
  struct udb_flag_bits _;
  int8_t B;
} udb_flags;
...
extern struct ADchannel udb_xaccel, udb_yaccel,
    udb_zaccel;  // x, y, and z accelerometer channels
extern struct ADchannel udb_xrate, udb_yrate,
    udb_zrate;                     // x, y, and z gyro channels
extern struct ADchannel udb_vref;  // reference voltage
extern struct ADchannel udb_analogInputs[];

#if (ANALOG_CURRENT_INPUT_CHANNEL != CHANNEL_UNUSED)
extern union longww battery_current;   // battery_current._.W1 is in tenths of
                                       // Amps
extern union longww battery_mAh_used;  // battery_mAh_used._.W1 is in mAh
#endif

#if (ANALOG_VOLTAGE_INPUT_CHANNEL != CHANNEL_UNUSED)
extern union longww battery_voltage;  // battery_voltage._.W1 is in tenths of
                                      // Volts
#endif

#if (ANALOG_RSSI_INPUT_CHANNEL != CHANNEL_UNUSED)
extern uint8_t rc_signal_strength;  // rc_signal_strength is 0-100 as percent of
                                    // full signal
#endif
...
boolean udb_gps_check_rate(int32_t rate);  // returns true if the rate arg is
                                           // the current rate
...
// Set the serial port data rate.  Use the UDB_BAUD_* constants defined in the
// Config*.h
// files.
void udb_serial_set_rate(int32_t rate);
boolean udb_serial_check_rate(int32_t rate);  // returns true if the rate arg is
                                              // the current rate

// Call this function to initiate sending a data to the serial port
void udb_serial_start_sending_data(void);

// Implement this callback to tell the UDB what byte is next to send on the
// serial port.
// Return -1 to stop sending data.
...
void osd_spi_write_byte(int8_t byte);      // Used for writing chars while in
                                           // auto-increment mode
void osd_spi_write_location(int16_t loc);  // Set where on screen to write the
                                           // next char
...
#define NUM_FLAG_ZERO_PADDED \
  1  // When num_digits > 0, left-pad with zeros instead of spaces
#define NUM_FLAG_SIGNED \
  2  // Reserve space for a - sign to the left of the number
void osd_spi_write_number(int32_t val, int8_t num_digits, int8_t decimal_places,
                          int8_t num_flags, int8_t header, int8_t footer);
...
// Write 1 byte to eeprom at address, or read 1 byte from address in eeprom into
// data
void eeprom_ByteWrite(uint16_t address, uint8_t data);
void eeprom_ByteRead(uint16_t address, uint8_t *data);

// Write numbytes of data to eeprom, starting at address. The write area can not
// span a
// page boundry.  Pages start on addresses of multiples of 64.
// Read numbytes of data from address in eeprom into data.  Note that there is
// no 1-page
