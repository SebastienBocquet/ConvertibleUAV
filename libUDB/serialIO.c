...
void udb_init_GPS(void) {
  ...
  // U1STAbits.UTXEN = 1;        //Bit10 TX pins controlled by periph (handled
  // below)
  ...
  _U1TXIP =
      INT_PRI_U1TX;  // Mid Range Interrupt Priority level, no urgent reason
  _U1RXIP =
      INT_PRI_U1RX;  // Mid Range Interrupt Priority level, no urgent reason
...
void udb_gps_set_rate(int32_t rate) { U1BRG = UDB_BAUD(rate); }

boolean udb_gps_check_rate(int32_t rate) { return (U1BRG == UDB_BAUD(rate)); }
...
void udb_init_USART(void) {
  ...
  // U2STAbits.UTXEN = 1;        //Bit10 TX pins controlled by periph (handled
  // below)
  ...
  _U2TXIP =
      INT_PRI_U2TX;  // Mid Range Interrupt Priority level, no urgent reason
  _U2RXIP =
      INT_PRI_U2RX;  // Mid Range Interrupt Priority level, no urgent reason
...
void udb_serial_set_rate(int32_t rate) { U2BRG = UDB_BAUD(rate); }
