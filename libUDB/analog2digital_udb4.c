...
//	Sampling and conversion is done automatically, so that all that needs to
//be done during
//	interrupt processing is to read the data out of the buffer.
//	Raw samples are taken approximately 500 per second per channel.
//	A first order digital lowpass filter with a time constant of about 32
//milliseconds
...
struct ADchannel udb_analogInputs[NUM_ANALOG_INPUTS];  // 0-indexed, unlike
                                                       // servo pwIn/Out/Trim
                                                       // arrays
#endif

// Number of locations for ADC buffer = 10 (AN1,4,6,9,10,11,15,16,17,18) x 1 =
// 10 words
// Align the buffer to 10 words or 20 bytes. This is needed for peripheral
// indirect mode
...
void udb_init_ADC(void) {
  ...
  AD1CON1bits.ASAM =
      1;  // ADC Sample Control: Sampling begins immediately after conversion
  ...
  AD1CON3bits.ADCS = 11;  // ADC Conversion Clock Tad=Tcy*(ADCS+1)= (1/40M)*12 =
                          // 0.3us (3333.3Khz)
...
void __attribute__((__interrupt__, __no_auto_psv__)) _DMA0Interrupt(void) {
  ...
  //	When there is a chance that read_gyros() and read_accel() will execute
  //soon,
...
#if (NUM_ANALOG_INPUTS >= 1)
    udb_analogInputs[0].value =
        __builtin_divsd(udb_analogInputs[0].sum, sample_count);
#endif
#if (NUM_ANALOG_INPUTS >= 2)
    udb_analogInputs[1].value =
        __builtin_divsd(udb_analogInputs[1].sum, sample_count);
#endif
#if (NUM_ANALOG_INPUTS >= 3)
    udb_analogInputs[2].value =
        __builtin_divsd(udb_analogInputs[2].sum, sample_count);
#endif
#if (NUM_ANALOG_INPUTS >= 4)
    udb_analogInputs[3].value =
        __builtin_divsd(udb_analogInputs[3].sum, sample_count);
#endif
  }
