...
struct ADchannel udb_vref;  // reference voltage (deprecated, here for MAVLink
                            // compatibility)

// Align the buffer. This is needed for peripheral indirect mode
#define NUM_AD_CHAN 7
__eds__ int16_t BufferA[NUM_AD_CHAN]
    __attribute__((eds, space(dma), aligned(32)));
__eds__ int16_t BufferB[NUM_AD_CHAN]
    __attribute__((eds, space(dma), aligned(32)));
...
void udb_init_ADC(void) {
  ...
  //	AD1CON3bits.ADCS    = 11;   // ADC Conversion Clock Tad=Tcy*(ADCS+1)=
  //(1/40M)*12 = 0.3us (3333.3Khz)
  ...
  uint16_t mask = ((1 << 6) | (1 << 7) | (1 << 8) | (1 << 9) | (1 << 13) |
                   (1 << 14) | (1 << 15));
  ANSELB = mask;
