...
int16_t udb_magOffset[3] = {
    0, 0, 0};  // magnetic offset in the body frame of reference
...
static uint8_t hmc5883read_index[] = {
    0x03};  // Address of the first register to read
static uint8_t hmc5883write_index[] = {
    0x00};  // Address of the first register to read

static uint8_t enableMagRead[] = {0x10, 0x20, 0x00};  // Continous measurament
static uint8_t enableMagCalibration[] = {
    0x11, 0x20, 0x01};  // Positive bias (Self Test) and single measurament
...
void rxMagnetometer(magnetometer_callback_funcptr callback)  // service the
                                                             // magnetometer
{
  ...
  if (magCalibPause == 0) {
    ...
    switch (magMessage) {
      case 1:  // read the magnetometer in case it is still sending data, so as
               // to NACK it
        I2C_Read(HMC5883_COMMAND, hmc5883read_index, 1, magreg, 6,
                 &I2C_callback, I2C_MODE_WRITE_ADDR_READ);
        break;
      case 2:  // put magnetomter into the power up defaults on a reset
        I2C_Write(HMC5883_COMMAND, hmc5883write_index, 1, resetMagnetometer, 3,
                  NULL);
        break;
      ...
      case 4:  // enable the calibration process
        magCalibPause = 2;
        I2C_Write(HMC5883_COMMAND, hmc5883write_index, 1, enableMagCalibration,
                  3, NULL);
        break;
      ...
      case 6:  // enable normal continuous readings
        I2C_Write(HMC5883_COMMAND, hmc5883write_index, 1, enableMagRead, 3,
                  NULL);
        break;
...
static void I2C_callback(boolean I2CtrxOK) {
  ...
  if (I2CtrxOK == true) {
    ...
    if (magMessage == 7) {
      udb_magFieldBody[0] =
          MAG_X_SIGN((__builtin_mulsu((magFieldRaw[MAG_X_AXIS]),
                                      magGain[MAG_X_AXIS])) >>
                     14) -
          (udb_magOffset[0] >> 1);
      udb_magFieldBody[1] =
          MAG_Y_SIGN((__builtin_mulsu((magFieldRaw[MAG_Y_AXIS]),
                                      magGain[MAG_Y_AXIS])) >>
                     14) -
          (udb_magOffset[1] >> 1);
      udb_magFieldBody[2] =
          MAG_Z_SIGN((__builtin_mulsu((magFieldRaw[MAG_Z_AXIS]),
                                      magGain[MAG_Z_AXIS])) >>
                     14) -
          (udb_magOffset[2] >> 1);
    ...
    } else if (magMessage == 5)  // Calibration data
    {
      ...
      for (vectorIndex = 0; vectorIndex < 3; vectorIndex++) {
        ...
        if ((magFieldRaw[vectorIndex] > MAGNETICMINIMUM) &&
            (magFieldRaw[vectorIndex] < MAGNETICMAXIMUM)) {
          magGain[vectorIndex] = __builtin_divud(((int32_t)(MAG_GAIN * RMAX)),
                                                 magFieldRaw[vectorIndex]);
        } else {
