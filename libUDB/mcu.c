...
// FGS
#pragma config GWRP = \
    OFF  // General Segment Write-Protect bit (General Segment may be written)
#pragma config GSS = OFF   // General Segment Code-Protect bit (General Segment
                           // Code protect is disabled)
#pragma config GSSK = OFF  // General Segment Key bits (General Segment Write
                           // Protection and Code Protection is Disabled)

// FOSCSEL
#pragma config FNOSC = PRIPLL  // Initial Oscillator Source Selection Bits
...
// FOSC
#pragma config POSCMD = \
    XT  // Primary Oscillator Mode Select bits (XT Crystal Oscillator Mode)
#pragma config OSCIOFNC = OFF  // OSC2 Pin Function bit (OSC2 is clock output)
#pragma config IOL1WAY = ON  // Peripheral pin select configuration (Allow only
                             // one reconfiguration)
#pragma config FCKSM = CSDCMD     // Clock Switching Mode bits (Both Clock
                                  // switching and Fail-safe Clock Monitor are
                                  // disabled)

// FWDT
#pragma config WDTPOST = PS32768  // Watchdog Timer Postscaler Bits (1:32,768)
#pragma config WDTPRE = PR128     // Watchdog Timer Prescaler bit (1:128)
#pragma config PLLKEN = ON   // PLL Lock Wait Enable bit (Clock switch to PLL
                             // source will wait until the PLL lock signal is
                             // valid.)
#pragma config WINDIS = OFF  // Watchdog Timer Window Enable bit (Watchdog Timer
                             // in Non-Window mode)
#pragma config FWDTEN = OFF  // Watchdog Timer Enable bit (Watchdog timer
                             // enabled/disabled by user software)

// FPOR
#pragma config FPWRT = PWR128  // Power-on Reset Timer Value Select bits (128ms)
#pragma config BOREN = \
    ON  // Brown-out Reset (BOR) Detection Enable bit (BOR is enabled)
#pragma config ALTI2C1 = ON  // Alternate I2C pins for I2C1 (ASDA1/ASCK1 pins
                             // are selected as the I/O pins for I2C1)
#pragma config ALTI2C2 = ON  // Alternate I2C pins for I2C2 (ASDA2/ASCK2 pins
                             // are selected as the I/O pins for I2C2)

// FICD
#pragma config ICS = PGD3  // ICD Communication Channel Select bits (Communicate
                           // on PGEC3 and PGED3)
#pragma config RSTPRI = PF   // Reset Target Vector Select bit (Device will
                             // obtain reset instruction from Primary flash)
#pragma config JTAGEN = OFF  // JTAG Enable bit (JTAG is disabled)

// FAS
#pragma config AWRP = OFF    // Auxiliary Segment Write-protect bit (Auxiliary
                             // program memory is not write-protected)
#pragma config APL = OFF   // Auxiliary Segment Code-protect bit (Aux Flash Code
                           // protect is disabled)
#pragma config APLK = OFF  // Auxiliary Segment Key bits (Aux Flash Write
                           // Protection and Code Protection is Disabled)
...
void configurePPS(void) {
  ...
  // in master mode, SS is not used by the SPI module; configure as GP output
  // instead
  // LATE4 is SS1
...
void configureDigitalIO(void)  // UDB4 and UDB5 boards
{
  _TRISD8 = 1;
#if (USE_PPM_INPUT == 0)
  _TRISD9 = _TRISD10 = _TRISD11 = _TRISD12 = _TRISD13 = _TRISD14 = _TRISD15 =
      _TRISD8;
#endif
...
#error Invalid MIPS Configuration
  ...
  if (_SWR == 1) {
    printf(
        "S/W Reset: trap_flags %04x, trap_source %04x%04x, osc_fail_count "
        "%u\r\n",
        trap_flags, (unsigned int)(trap_source >> 16),
        (unsigned int)(trap_source & 0xffff), osc_fail_count);
  }
#endif  // CONSOLE_UART
}
