...
boolean udb_nv_memory_read(uint8_t* rdBuffer, uint16_t address, uint16_t rdSize,
  ...
  if (I2C1_Read(MCP24LC256_COMMAND, commandData, 2, rdBuffer, rdSize,
                &MCP24LC256_callback, 0) == false) {
...
boolean MCP24LC256_write_chunk() {
  ...
  if (writeSize > 0x40) writeSize = 0x40;
  ...
  if (I2C1_Write(MCP24LC256_COMMAND, commandData, 2, MCP24LC256_pwrBuffer,
                 writeSize, &MCP24LC256_callback) == false) {
    MCP24LC256_Timer = 0;
