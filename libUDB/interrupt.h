...
#define INT_PRI_T1 \
  6  // background.c : high priority HEARTBEAT_HZ heartbeat of libUDB
...
#define INT_PRI_T5 \
  6  // background.c : high priority, but ISR is very short - used to measure
     // CPU usage.
#define INT_PRI_T6 \
  3  // background.c : trigger from the high priority heartbeat ISR to start all
     // the HEARTBEAT_HZ processing at a lower priority - NOTE: timer 6 is not
     // actually being used
#define INT_PRI_T7 \
  2  // background.c : used to trigger background tasks such as navigation
     // processing after binary data is received from the GPS
...
// ISR in the middle of a dsp math call, CORCON can be set to an unexpected
// value.
