...
void osd_reset(void) {
  osd_spi_write(0x00, 0x42);  // VM0: enable display of PAL OSD image, force
                              // software reset
  __delay32(400000 * SF);
  //		osd_spi_write(0x00, 0x08) ;	// VM0: enable display of NTSC OSD
  //image
  osd_spi_write(0x00, 0x48);  // VM0: enable display of PAL OSD image

  //		osd_spi_write(0x03, 0x00) ;	// VOS set to +15 pixels (farthest
  //up)
  //		osd_spi_write(0x03, 0x10) ;	// VOS set to +-0 pixels (no shift,
  //default)
...
void spi_write_raw_byte(uint8_t byte) {
  ...
  for (SPICount = 0; SPICount < 8;
       SPICount++)  // Prepare to clock out the Address byte
  {
    uint8_t outBit = ((byte & 0x80) !=
                      0);  // Check for a 1 and set the MOSI line appropriately
    if (outBit)
      OSD_MOSI =
          1;  // Write this bit using the bit-set / bit-clear instrictions
    else
      OSD_MOSI = 0;
    ...
    Nop();
    Nop();
    Nop();        // Kill some time with SCK high to make a more solid pulse
...
uint8_t osd_spi_read(int8_t addr)
{
        ...
        OSD_CS = 1 ;				// Make sure we start with
active-low CS high
        OSD_SCK = 0 ;				// and CK low

        OSD_CS = 0 ;				// Set active-low CS low to
start the SPI cycle
        spi_write_raw_byte(addr) ;	// Send the Address
        OSD_MOSI = 0 ;
        SPIData = spi_read_raw_byte() ;	// Send the data

        OSD_CS = 1 ;				// Set active-low CS high to
end the SPI cycle

        Nop(); Nop(); Nop(); Nop();	// Kill some time with CS high to
make a more solid pulse
...
void osd_spi_write_string(const uint8_t *str) {
  ...
  while (1) {
    osd_spi_write_byte(*str);  // Disables auto-increment mode when sending 0xFF
...
void osd_spi_write_vertical_string_at_location(int16_t loc,
                                               const uint8_t *str) {
  while (1) {
    if (*str == 0xFF) break;
...
void osd_spi_write_number(int32_t val, int8_t num_digits, int8_t decimal_places,
  ...
  if (header) osd_spi_write_byte(header);
  ...
  if (footer) osd_spi_write_byte(footer);

  if (num_digits == 0) osd_spi_write_byte(0x00);
