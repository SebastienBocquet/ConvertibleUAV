...
int16_t lidar_height_to_ground;  // calculated distance to ground in Earth's Z
                                 // Plane allowing for tilt
int16_t failure_lidar_distance = OUT_OF_RANGE_DISTANCE;
unsigned char lidar_good_sample_count =
    0;  // Tracks the number of consequtive good samples up until
        // SONAR_SAMPLE_THRESHOLD is reached.
unsigned char lidar_no_readings_count = 0;  // Tracks number of UDB frames since
...
#define LIDAR_SAMPLE_THRESHOLD \
  1  // Number of readings before code deems "certain" of a true reading.
...
void calculate_lidar_height_above_ground(void) {
  ...
  if (udb_flags._.lidar_updated == 1) {
    ...
    // PWM input
    // should be : udb_pwm_lidar (in micro sec) / 10. Here there must be a
    // scaling * 2 of the pwm pulse duration, so we divide by 20
    // udb_pwm_lidar is an unsigned int, so it range between 0 to 65535. So
    // lidar distance can range between 0 to 65535/20=3276cm = 32.76m.
    lidar_distance = (int16_t)(udb_pwm_lidar / 20);
#endif

    if (lidar_distance > USEABLE_LIDAR_DISTANCE ||
        lidar_distance < LIDAR_MINIMUM_DISTANCE) {
