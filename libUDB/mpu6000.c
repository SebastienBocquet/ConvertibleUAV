...
struct ADchannel udb_xaccel, udb_yaccel,
...
#error Invalid MIPS Configuration
  ...
  // SAMPLE RATE
  writeMPUSPIreg16(MPUREG_SMPLRT_DIV,
                   4);  // Sample rate = 200Hz  Fsample= 1Khz/(N+1) = 200Hz

  // scaling & DLPF
  writeMPUSPIreg16(MPUREG_CONFIG, BITS_DLPF_CFG_42HZ);

  //	writeMPUSPIreg16(MPUREG_GYRO_CONFIG, BITS_FS_2000DPS);  // Gyro scale
  //2000º/s
  writeMPUSPIreg16(MPUREG_GYRO_CONFIG, BITS_FS_500DPS);  // Gyro scale 500º/s

#if (ACCEL_RANGE == 2)
  writeMPUSPIreg16(MPUREG_ACCEL_CONFIG,
                   BITS_FS_2G);  // Accel scele 2g, g = 8192
#elif(ACCEL_RANGE == 4)
...
//	writeMPUSPIreg16(MPUREG_ACCEL_CONFIG, BITS_FS_8G); // Accel scale g = 4096
  ...
  writeMPUSPIreg16(MPUREG_INT_PIN_CFG,
                   BIT_INT_LEVEL | BIT_INT_RD_CLEAR);  // INT: Clear on any read
  writeMPUSPIreg16(MPUREG_INT_ENABLE, BIT_DATA_RDY_EN);  // INT: Raw data ready

// Bump the SPI clock up towards 20 MHz for ongoing sensor and interrupt
// register reads
// Warning: the SPI limit on the dsPIC is 10 Mhz
...
#error Invalid MIPS Configuration
  ...
  _TRISMPUINT =
      1;  // this is probably already taken care of in mcu.c for most boards
...
void process_MPU_data(void) {
  ...
  //  This version of the MPU interface writes and reads gyro and accelerometer
  // values asynchronously.
  //  This was the fastest way to revise the software.
  //  MPU data is being read at 200 Hz, IMU and control loop runs at 40 Hz.
  //  4 out of 5 samples are being ignored. IMU gets the most recent set of
  // samples.
  //  Eventually, we will want to run write-read synchronously, and run the IMU
  // at 200 Hz, using every sample.
...
void MPU6000_read(void) {
  ...
  mpuCnt++;
  // Non-blocking read of 7 words of data from MPU, starting with X
  // acceleration, and then call process_MPU_data
...
void MPU6000_print(void) {
  printf("%06u axyz %06i %06i %06i gxyz %06i %06i %06i t %u\r\n", mpuCnt,
         mpu_data[0], mpu_data[1], mpu_data[2], mpu_data[4], mpu_data[5],
         mpu_data[6], mpu_data[3]);
}
