...
//	One of the channels is also used to validate pulse widths to detect loss
//of radio.

//	The pulse width inputs can be directly converted to units of pulse width
//outputs to control
//	the servos by simply dividing by 2. (need to check validity of this
//statement - RobD)
...
#define IC_INIT(x, y, z) _IC_INIT(x, y, z)
  ...
  if ((NUM_INPUTS > 6) || (USE_SONAR_ON_PWM_INPUT_7 == 1))
    IC_INIT(7, REGTOK1, REGTOK2);
  if ((NUM_INPUTS > 7) || (USE_LIDAR_ON_PWM_INPUT_8 == 1))
    IC_INIT(8, REGTOK1, REGTOK2);
#else
...
void set_udb_pwIn(int pwm, int index) {
  ...
  if (MODE_SWITCH_INPUT_CHANNEL == index) {
    ...
    if (udb_pwIn[MODE_SWITCH_INPUT_CHANNEL] < MODE_SWITCH_THRESHOLD_LOW) {
      // if mode is in low mode, use pwm values that came in from external
      // source
...
#define _IC_HANDLER(x, y, z)                                              \
  void __attribute__((__interrupt__, __no_auto_psv__)) _IC##x##Interrupt( \
      void) {                                                             \
    ...
    uint16_t time = 0;                                                    \
    _IC##x##IF = 0;                                                       \
    while (IC##x##CO##y##bits.ICBNE) time = IC##x##BUF;                   \
    if (z)                                                                \
      rise = time;                                                        \
    else                                                                  \
