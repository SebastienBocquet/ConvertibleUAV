...
// Configure SPI1 module in master mode
  ...
  SPICON1Value = ENABLE_SDO_PIN & SPI_MODE16_ON & ENABLE_SCK_PIN & SPI_SMP_OFF &
                 SPI_CKE_OFF & SLAVE_ENABLE_OFF & CLK_POL_ACTIVE_LOW &
                 MASTER_ENABLE_ON & secPre & priPre;
  SPICON2Value =
      FRAME_ENABLE_OFF & FRAME_SYNC_OUTPUT;  // & FIFO_BUFFER_DISABLE;
  SPISTATValue = SPI_ENABLE & SPI_IDLE_CON & SPI_RX_OVFLOW_CLR & BUF_INT_SEL_5;
#else
  SPICON1Value = ENABLE_SDO_PIN & SPI_MODE16_ON & ENABLE_SCK_PIN & SPI_SMP_ON &
                 SPI_CKE_OFF & SLAVE_ENABLE_OFF & CLK_POL_ACTIVE_LOW &
                 MASTER_ENABLE_ON & secPre & priPre;
...
void readSPI1_burst16n(uint16_t data[], int16_t n, uint16_t addr,
                       void (*call_back)(void)) {
  uint16_t i;
  // assert chip select
  SPI1_SS = 0;
...
void readSPI2_burst16n(uint16_t data[], int16_t n, uint16_t addr,
                       void (*call_back)(void)) {
  uint16_t SPIBUF;
...
void __attribute__((__interrupt__, __no_auto_psv__)) _SPI2Interrupt(void) {
  uint16_t SPIBUF;
  // clear the interrupt flag as soon as possible so as to not miss any
  // interrupts
  _SPI2IF = 0;
