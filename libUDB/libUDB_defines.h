...
#define GREEN_BOARD \
  2  // green board with Analog Devices 75 degree/second gyros (deprecated)
#define UDB3_BOARD \
  3  // red board with daughter boards 500 degree/second Invensense gyros
     // (deprecated)
#define RUSTYS_BOARD \
  4  // Red board with Rusty's IXZ-500_RAD2a patch board (deprecated)
#define UDB4_BOARD \
  5  // board with dsPIC33 and integrally mounted 500 degree/second Invensense
     // gyros
...
// Dead reckoning
// DEADRECKONING 0 selects the GPS to perform navigation, at the GPS update
// rate.
// DEADRECKONING 1 selects the dead reckoning computations to perform
// navigation, at 40 Hz.
...
// Wind estimation is done using a mathematical model developed by William
// Premerlani.
// Every time the plane performs a significant turn, the plane estimates the
// wind.
...
#define CHANNEL_UNUSED \
  0  // udb_pwIn[0], udb_pwOut[0], etc. are not used, but allow lazy code
     // everywhere else  :)
#define CHANNEL_1 1
#define CHANNEL_2 2
#define CHANNEL_3 3
...
// Constants
#define RMAX 16384  // 0b0100000000000000        // 1.0 in 2.14 fractional
                    // format
...
#define MAX_CURRENT \
  1788  // 178.8 Amps max for the sensor from SparkFun (in tenths of Amps)
#define CURRENT_SENSOR_OFFSET 0  // Add 0 Amp to whatever value we sense
#define MAX_USEABLE_MAH \
  1680  // For a 2400mAh battery, we set the max useable mAH to 2400 * 0.7

#define MAX_VOLTAGE \
  518  // 51.8 Volts max for the sensor from SparkFun (in tenths of Volts)
#define VOLTAGE_SENSOR_OFFSET 0  // Add 0.0 Volts to whatever value we sense
#define LOW_VOLTAGE \
  95  // for a 3S battery, telemetry shows around 9.9V for 25A draw, with
      // exursions between 9.5V to 10.2V.
// So 9.5V is set as the lower limit
#define NB_LOW_VOLTAGE_SECONDS \
  3  // if battery voltage is low during this number of seconds, then the
     // emergency landing is activated
