...
#define I2C1_NORMAL                         \
  (((I2C1CON & 0b0000000000011111) == 0) && \
   ((I2C1STAT & 0b0100010011000001) == 0))
...
void I2C1_trigger_service(void) { trigger_event(I2C1_service_handle); }
...
static void serviceI2C1(void)  // service the I2C
{
  if (_I2C1EN == 0)  // I2C is off
  {
    ...
    // Put something here to reset state machine.  Make sure attached servies
    // exit nicely.
  }
}
...
boolean I2C1_Write(uint8_t address, uint8_t* pcommandData,
                   uint8_t commandDataSize, uint8_t* ptxData, uint16_t txSize,
...
boolean I2C1_Read(uint8_t address, uint8_t* pcommandData,
                  uint8_t commandDataSize, uint8_t* prxData, uint16_t rxSize,
                  I2C_callbackFunc pCallback, uint16_t I2C_mode) {
  if (!I2C1_CheckAvailable()) return false;
...
static void I2C1_doneWrite(void) {
  I2C1_Busy = false;
  if (pI2C_callback != NULL) pI2C_callback(true);
}
...
static void I2C1_Failed(void) {
  ...
  I2C1CONbits.PEN = 1;
  I2C1_Busy = false;
  if (pI2C_callback != NULL) pI2C_callback(false);
}
