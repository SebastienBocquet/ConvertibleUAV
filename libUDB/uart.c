...
#else
#define BAUDRATEREG                                                   \
  (((GetSystemClock() / 2) + (BRG_DIVX / 2 * BAUDRATEX)) / BRG_DIVX / \
       BAUDRATEX -                                                    \
   1)
#endif
#elif defined(__PIC32MX__)
#define BAUDRATEREG                                                 \
  ((GetPeripheralClock() + (BRG_DIVX / 2 * BAUDRATEX)) / BRG_DIVX / \
       BAUDRATEX -                                                  \
   1)
#else
...
#define BAUD_ERROR                                     \
  ((BAUD_ACTUAL > BAUDRATEX) ? BAUD_ACTUAL - BAUDRATEX \
                             : BAUDRATEX - BAUD_ACTUAL)
...
Overview:
        This function converts decimal data into a string and outputs it to
UART.
