...
#define GGAIN                      \
  SCALEGYRO*(240 / HEARTBEAT_HZ) * \
      (RMAX * 0.025)  // integration multiplier for gyros at HEARTBEAT_HZ
...
fractional spin_axis[] = {0, 0, RMAX};
     ...
     BOARD_TYPE == UDB4_BOARD || BOARD_TYPE == UDB5_BOARD)
...
static fractional rmatDelayCompensated[] = {RMAX, 0, 0,     0, 0,
                                            RMAX, 0, -RMAX, 0};
...
void read_gyros(void) {
  ...
  if (spin_rate_over_2 > 0) {
    spin_axis[0] =
        __builtin_divsd(((int32_t)omegagyro[0]) << 13, spin_rate_over_2);
    spin_axis[1] =
        __builtin_divsd(((int32_t)omegagyro[1]) << 13, spin_rate_over_2);
    spin_axis[2] =
        __builtin_divsd(((int32_t)omegagyro[2]) << 13, spin_rate_over_2);
  }
}
...
void read_accel(void) {
  ...
  accelEarth[2] =
      -((int16_t)GRAVITY) + (VectorDotProduct(3, &rmat[6], gplane) << 1);

  //	accelEarthFiltered[0].WW += ((((int32_t)accelEarth[0])<<16) -
  //accelEarthFiltered[0].WW)>>5;
  //	accelEarthFiltered[1].WW += ((((int32_t)accelEarth[1])<<16) -
  //accelEarthFiltered[1].WW)>>5;
  //	accelEarthFiltered[2].WW += ((((int32_t)accelEarth[2])<<16) -
  //accelEarthFiltered[2].WW)>>5;
}
...
static void normalize(void) {
  ...
  error =
      -VectorDotProduct(3, &rmat[0], &rmat[3]);  // note, 1/2 is built into 2.14
...
static void roll_pitch_drift(void) { VectorCross(errorRP, gplane, &rmat[6]); }
...
static void yaw_drift(void) {
  ...
  if (dcm_flags._.yaw_req) {
    if (ground_velocity_magnitudeXY > GPS_SPEED_MIN) {
      ...
      // *** Note: this accomplishes multiplication rmat transpose times
      // errorYawground!!
...
static void quaternion_adjust(fractional quaternion[], fractional direction[]) {
  ...
  // the cross product is left out, theory and test both show it should not be
  // used.
  ...
  // change is cosine is 1/2 of the dot product of first 3 elements of
  // quaternion
  // with the increment. The 1/2 is built into the dot product.
  delta_cos = -VectorDotProduct(3, quaternion, increment);
  // the change in the first 3 elements is 1/2 of the 4 element times the
  // increment.
  ...
  quaternion[0] =
      __builtin_divsd(__builtin_mulsu(quaternion[0], RMAX), magnitude);
  quaternion[1] =
      __builtin_divsd(__builtin_mulsu(quaternion[1], RMAX), magnitude);
  quaternion[2] =
      __builtin_divsd(__builtin_mulsu(quaternion[2], RMAX), magnitude);
  quaternion[3] =
      __builtin_divsd(__builtin_mulsu(quaternion[3], RMAX), magnitude);
}

static void RotVector2RotMat(fractional rotation_matrix[],
                             fractional rotation_vector[]) {
  // rotation vector represents a rotation in vector form
  // around an axis equal to the normalized value of the vector.
  // It is assumed that rotation_vector already includes a factor of
  // sin(alpha/2)
  ...
  for (matrix_index = 0; matrix_index < 3; matrix_index++) {
    sin_half_alpha_sqr.WW += __builtin_mulss(rotation_vector[matrix_index],
                                             rotation_vector[matrix_index]);
  }
  ...
  VectorScale(3, cos_half_alpha_rotation_vector, rotation_vector,
              cos_half_alpha);
  for (matrix_index = 0; matrix_index < 3; matrix_index++) {
...
static void mag_drift(void) {
  ...
  if (dcm_flags._.mag_drift_req) {
    ...
    magFieldBodyMagnitude = vector3_mag(
    ...
    VectorScale(3, offsetEstimate, vectorBuffer,
                magFieldBodyMagnitude - magFieldBodyMagnitudePrevious);
    ...
    vectorBuffer[0] = VectorDotProduct(3, &rmatBufferA[0], udb_magFieldBody)
                      << 1;
    vectorBuffer[1] = VectorDotProduct(3, &rmatBufferA[3], udb_magFieldBody)
                      << 1;
    vectorBuffer[2] = VectorDotProduct(3, &rmatBufferA[6], udb_magFieldBody)
                      << 1;
    ...
    magFieldEarth[0] =
        VectorDotProduct(3, &rmatDelayCompensated[0], udb_magFieldBody) << 1;
    magFieldEarth[1] =
        VectorDotProduct(3, &rmatDelayCompensated[3], udb_magFieldBody) << 1;
    magFieldEarth[2] =
        VectorDotProduct(3, &rmatDelayCompensated[6], udb_magFieldBody) << 1;
...
// Use the magnetometer to detect yaw drift
    ...
    VectorCross(magAlignmentError, magFieldEarthNormalizedPrevious,
                magFieldEarthNormalized);
    ...
    // Convert to a rotation vector, take advantage of 1/2 from the previous
    // step
    ...
    VectorCross(magAlignmentAdjustment, R2TAlignmentErrorR1,
                R2TR1RotationVector);
...
#ifdef USE_DEBUG_IO
  printf("magno %u %u %u\r\n", udb_magFieldBody[0], udb_magFieldBody[1],
         udb_magFieldBody[2]);
#endif
}
...
static void PI_feedback(void) {
  ...
  // boost the KPs at high spin rate, to compensate for increased error due to
  // calibration error
  ...
  if (spin_rate < ((uint16_t)(50.0 * DEGPERSEC))) {
    ...
    kprollpitch =
        ((uint16_t)(KPROLLPITCH * 8.0 / (50.0 * DEGPERSEC))) * (spin_rate >> 3);
  } else {
  ...
  // turn off the offset integrator while spinning, it doesn't work in that
  // case,
  // and it only causes trouble.

  if (spin_rate < ((uint16_t)(MAXIMUM_SPIN_DCM_INTEGRAL * DEGPERSEC))) {
    gyroCorrectionIntegral[0].WW +=
        (__builtin_mulss(errorRP[0], KIROLLPITCH) >> 3);
    gyroCorrectionIntegral[1].WW +=
        (__builtin_mulss(errorRP[1], KIROLLPITCH) >> 3);
    gyroCorrectionIntegral[2].WW +=
        (__builtin_mulss(errorRP[2], KIROLLPITCH) >> 3);

    gyroCorrectionIntegral[0].WW +=
        (__builtin_mulss(errorYawplane[0], KIYAW) >> 3);
    gyroCorrectionIntegral[1].WW +=
        (__builtin_mulss(errorYawplane[1], KIYAW) >> 3);
    gyroCorrectionIntegral[2].WW +=
        (__builtin_mulss(errorYawplane[2], KIYAW) >> 3);
  }
...
static void calibrate_gyros(void) {
  ...
  if (spin_rate > (uint16_t)(MINIMUM_SPIN_RATE_GYRO_CALIB * DEGPERSEC)) {
    ...
    VectorMultiply(3, omegacorrPweighted, spin_axis,
                   omegacorrP);  // includes 1/2

    calib_accum = __builtin_mulsu(omegacorrPweighted[0],
                                  (uint16_t)(0.025 * GGAIN / GYRO_CALIB_TAU));
    gain_change = __builtin_divsd(calib_accum, spin_rate_over2);
    ggain[0] = adjust_gyro_gain(ggain[0], gain_change);

    calib_accum = __builtin_mulsu(omegacorrPweighted[1],
                                  (uint16_t)(0.025 * GGAIN / GYRO_CALIB_TAU));
    gain_change = __builtin_divsd(calib_accum, spin_rate_over2);
    ggain[1] = adjust_gyro_gain(ggain[1], gain_change);

    calib_accum = __builtin_mulsu(omegacorrPweighted[2],
                                  (uint16_t)(0.025 * GGAIN / GYRO_CALIB_TAU));
    gain_change = __builtin_divsd(calib_accum, spin_rate_over2);
