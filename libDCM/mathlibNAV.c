...
int16_t cosine(int8_t angle) { return (sine(angle + 64)); }
...
void rotate_2D_vector_by_vector(int16_t vector[2], int16_t rotate[2]) {
  ...
  //	rotate is RMAX*[ cosine(theta), sine(theta) ], theta is the desired
  //rotation angle
  //	upon exit, the vector [ x, y ] will be rotated by the angle theta.
  //	theta is positive in the counter clockwise direction.
  //	This routine can also be used to do a complex multiply, with 1/RMAX
  //scaling,
  //	and where vector and rotate are viewed as complex numbers
  int16_t newx, newy;
  union longww accum;
  accum.WW = ((__builtin_mulss(rotate[0], vector[0]) -
               __builtin_mulss(rotate[1], vector[1]))
              << 2);
  newx = accum._.W1;
  accum.WW = ((__builtin_mulss(rotate[1], vector[0]) +
               __builtin_mulss(rotate[0], vector[1]))
              << 2);
  newy = accum._.W1;
  vector[0] = newx;
  vector[1] = newy;
...
void rotate(struct relative2D *xy, int8_t angle) {
  ...
  //  A mathematical angle of plus or minus pi is represented digitally as plus
  // or minus 128.
  ...
  cosang = cosine(angle);
  accum.WW =
      ((__builtin_mulss(cosang, xy->x) - __builtin_mulss(sinang, xy->y)) << 2);
  newx = accum._.W1;
  accum.WW =
      ((__builtin_mulss(sinang, xy->x) + __builtin_mulss(cosang, xy->y)) << 2);
  newy = accum._.W1;
  xy->x = newx;
  xy->y = newy;
}
...
int8_t rect_to_polar(struct relative2D *xy) {
  //  Convert from rectangular to polar coordinates using "CORDIC" arithmetic,
  // which is basically
  //  a binary search for the angle.
  //  As a by product, the xy is rotated onto the x axis, so that y is driven to
  // zero,
...
uint16_t sqrt_int(uint16_t sqr) {
  ...
  sqr = sqr / 2;  // for convenience, Herons formula is result = (result +
                  // sqr/result) / 2
  while (iterations) {
    iterations--;
  ...
  result = result >>
           binary_point;  // shift result right to account for shift left of sqr
  return result;
}
...
uint16_t sqrt_long(uint32_t sqr) {
  ...
  uint16_t binary_point = 0;
  uint16_t result =
      65535;  // need to start high and work down to avoid overflow in divud
  ...
  sqr = sqr >> 1;  // for convenience, Herons formula is result = (result +
                   // sqr/result) / 2
  while (iterations) {
    iterations--;
...
uint16_t vector3_mag(int16_t x, int16_t y, int16_t z) {
  uint32_t magsqr;
  magsqr =
      __builtin_mulss(x, x) + __builtin_mulss(y, y) + __builtin_mulss(z, z);
  return sqrt_long(magsqr);
}
...
//}
//
// void matrix22_vector_multiply(int32_t *result, int32_t input1[], int32_t
// input2[])
//{
...
int16_t compute_pi_block(int16_t input, int16_t target, uint16_t kp,
                         uint16_t ki, int32_t *error_integral,
                         int16_t heartbeat_hz, int32_t limitintegral,
                         boolean integrate) {
  int16_t output;
  int32_t tmp;
...
int16_t IIR_Filter(int32_t *filter, int16_t sample, int8_t delta_filter) {
  ...
  *filter =
      (local_sample * (256 - delta_filter) + (*filter) * delta_filter) / 256;
...
int16_t exponential_filter(int16_t x, float *x_filtered, float invdeltafilter) {
  ...
  *x_filtered =
      ((float)x * invdeltafilter +
       (float)(*x_filtered) * ((float)(HEARTBEAT_HZ) - invdeltafilter)) *
      dt;
  return (int16_t) * x_filtered;
}

int32_t exponential_filter32(int32_t x, float *x_filtered,
                             float invdeltafilter) {
  float dt = 1.0 / (float)(HEARTBEAT_HZ);
  *x_filtered =
      ((float)x * invdeltafilter +
       (float)(*x_filtered) * ((float)(HEARTBEAT_HZ) - invdeltafilter)) *
...
int16_t sga_prim(int16_t current_value) {
  ...
  for (i = -SGA_MID; i <= SGA_MID; i++) {
    sum += history_SGA[i + SGA_MID] *
           sga_prim_coefficients[SGA_PRIM_INDEX][i + SGA_MAX_MID];
  }
