...
#include "libDCM_internal.h" #include "defines.h"

#if (GPS_TYPE == GPS_STD)

//	Parse the GPS messages, using the binary interface.
//	The parser uses a state machine implemented via a pointer to a function.
//	Binary values received from the GPS are directed to program variables
//via a table
//	of pointers to the variable locations.
//	Unions of structures are used to be able to access the variables as
//long, ints, or bytes.
...
const uint8_t mode[] = {0x86, 0x00, 0x00, 0x4B, 0x00, 0x08, 0x01, 0x00, 0x00};
...
uint8_t* const msg41parse[] = {
    &nav_valid_._.B1,   &nav_valid_._.B0,   &nav_type_._.B1,
    &nav_type_._.B0,
    // &un, &un, &un, &un, &un, &un,
    &week_no_._.B1,     &week_no_._.B0,     &tow_.__.B3,
    &tow_.__.B2,        &tow_.__.B1,        &tow_.__.B0,
    &un,                &un,                &un,
    &un,                &un,                &un,
    &un,                &un,                &un,
    &un,                &un,                &un,
    &lat_gps_.__.B3,    &lat_gps_.__.B2,    &lat_gps_.__.B1,
    &lat_gps_.__.B0,    &long_gps_.__.B3,   &long_gps_.__.B2,
    &long_gps_.__.B1,   &long_gps_.__.B0,   &un,
    &un,                &un,                &un,
    &alt_sl_gps_.__.B3, &alt_sl_gps_.__.B2, &alt_sl_gps_.__.B1,
    &alt_sl_gps_.__.B0, &un,                &sog_gps_._.B1,
    &sog_gps_._.B0,     &cog_gps_._.B1,     &cog_gps_._.B0,
    &un,                &un,                &climb_gps_._.B1,
    &climb_gps_._.B0,   &un,                &un,
    &un,                &un,                &un,
    &un,                &un,                &un,
    &un,                &un,                &un,
    &un,                &un,                &un,
    ...
    &un,                &un,                &un,
    &un,                &un,                &svs_,
    &hdop_,             &un,                &checksum_._.B1,
    &checksum_._.B0};

//	if nav_valid is zero, there is valid GPS data that can be used for
//navigation.
boolean gps_nav_valid(void) { return (nav_valid_.BB == 0); }
...
//	Used for debugging purposes, converts to HEX and outputs to the
debugging USART
...
//	The parsing routines follow. Each routine is named for the state in
//which the routine is applied.
//	States correspond to the portions of the binary messages.
//	For example, msg_B3 is the routine that is applied to the byte received
//after a B3 is received.
...
void msg_MSG41(uint8_t gpschar) {
  if (payloadlength.BB > 0) {
    *msg41parse[store_index++] = gpschar;
    if (payloadlength.BB >
        2)  // Don't include the sent checksum bytes in the checksum calculation
    {
      calculated_checksum.BB += gpschar;
    }
    payloadlength.BB--;
...
void msg_B0(uint8_t gpschar) {
  if (gpschar == 0xB3) {
    ...
    if (calculated_checksum.BB != INVALID_CHECKSUM && checksum_.BB == masked) {
      udb_background_trigger();  // parsing is complete and valid, schedule
                                 // navigation
    }
    msg_parse = &msg_B3;
  } else {
