...
// If GPS data has not been received for this many state machine cycles,
// consider the GPS lock to be lost.
...
#define CENTRISCALE \
  ((int32_t)(((int64_t)519168.0) * GRAVITY) / ((int64_t)RADPERSEC* GRAVITYM))
// scale factor in multiplying omega times velocity to get centrifugal
// acceleration
...
#define WIND_NAV_AIR_SPEED_MIN \
  200  // Minimum airspeed in cm/sec for wind navigation to apply
#define GPS_SPEED_MIN \
  150  // Minimum ground speed in cm/sec to use GPS for yaw drift compensation
