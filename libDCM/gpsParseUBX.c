...
// Binary values received from the GPS are directed to program variables via a
// table
// of pointers to the variable locations.
// Unions of structures are used to be able to access the variables as long,
// ints, or bytes.
...
uint8_t* const msg_SOL_parse[] = {
    &tow_.__.B0,    &tow_.__.B1,    &tow_.__.B2,
    &tow_.__.B3,  // iTOW
    &un,            &un,            &un,
    &un,                             // fTOW
    ...
    &un,  // ecefZ
#if (HILSIM == 1 && MAG_YAW_DRIFT == 1)
    &magreg[1],     &magreg[0],     &magreg[3],
    &magreg[2],  // simulate the magnetometer with HILSIM, and use these slots
...
#if (HILSIM == 1 && MAG_YAW_DRIFT == 1)
    &magreg[5],     &magreg[4],     &un,
    &un,  // simulate the magnetometer with HILSIM, and use these slots
          // note: mag registers come out high:low from magnetometer
#else
    &un,            &un,            &un,         &un,  // sAcc
#endif
...
uint8_t* const msg_DOP_parse[] = {&un,         &un,         &un, &un,  // iTOW
                                  &un,         &un,                    // gDOP
                                  &un,         &un,                    // pDOP
                                  &un,         &un,                    // tDOP
                                  &un,         &un,                    // vDOP
...
uint8_t* const msg_BODYRATES_parse[] = {
    &p_sim_._.B0,
    &p_sim_._.B1,  // roll rate
    &q_sim_._.B0,
    &q_sim_._.B1,  // pitch rate
    &r_sim_._.B0,
    &r_sim_._.B1,  // yaw rate
    &g_a_x_sim_._.B0,
    &g_a_x_sim_._.B1,  // x accel reading (grav - accel, body frame)
    &g_a_y_sim_._.B0,
    &g_a_y_sim_._.B1,  // y accel reading (grav - accel, body frame)
    &g_a_z_sim_._.B0,
    &g_a_z_sim_._.B1,  // z accel reading (grav - accel, body frame)
};
#endif  // HILSIM
...
boolean gps_nav_valid(void) { return (nav_valid_ == 3); }
...
// Used for debugging purposes, converts to HEX and outputs to the debugging
USART
...
// The parsing routines follow. Each routine is named for the state in which the
// routine is applied.
// States correspond to the portions of the binary messages.
// For example, msg_B3 is the routine that is applied to the byte received after
// a B3 is received.
// If an A0 is received, the state machine transitions to the A0 state.

int16_t nmea_passthru_countdown =
    0;  // used by nmea_passthru to count how many more bytes are passed through
...
void nmea_passthru(uint8_t gpschar) {
  ...
  /* removed in favor of the line ending mechanism, see below. While this is
     compliant with
     (published) standards, the issue appears to center around the end of line.

          if (gpschar == '*')
          { // * indicates the start of the checksum, 2 characters remain in the
     message
...
void msg_B3(uint8_t gpschar) {
  if (gpschar == 0xB5) {
    ...
    msg_parse = &msg_SYNC1;
             ...
             udb_gps_check_rate(19200)) {
    nmea_passthru_countdown = 128;  // this limits the number of characters we
                                    // will passthrough. (Most lines are 60-80
                                    // chars long.)
...
void msg_ID(uint8_t gpschar) {
  // bin_out(0x05);
  payloadlength._.B0 =
      gpschar;  // UBX stored payload length in little endian order
  CK_A += gpschar;
  CK_B += CK_A;
  msg_parse = &msg_PL1;
}
  ...
  switch (msg_class) {
    case 0x01: {
      switch (msg_id) {
        ...
        case 0x12: {  // NAV-VELNED message
          ...
          msg_parse = &msg_VELNED;  // TODO: this does not look right (wipes out
                                    // error setting above) - RobD
          break;
        }
#if (HILSIM == 1)
        case 0xAB
    ...
    case 0x05: {
      switch (msg_id) {
        ...
        default: {  // There are no other messages in this class, so this is an
                    // error
          msg_parse = &msg_B3;
          break;
        }
...
void msg_POSLLH(uint8_t gpschar) {
  if (payloadlength.BB > 0) {
    *msg_POSLLH_parse[store_index++] = gpschar;
    ...
    // If the payload length is zero, we have received the entire payload, or
    // the payload length
    // was zero to start with. either way, the byte we just received is the
    // first checksum byte.
    // gpsoutchar2(0x08);
...
void msg_CS1(uint8_t gpschar) {
  ...
  if ((checksum._.B1 == CK_A) && (checksum._.B0 == CK_B)) {
    if (msg_id == 0x12) {
      ...
      // If we got the correct checksum for bodyrates, commit that data
      // immediately
      commit_bodyrate_data();
    }
#endif
  } else {
    gps_data_age = GPS_DATA_MAX_AGE + 1;  // if the checksum is wrong then the
                                          // data from this packet is invalid.
...
void commit_gps_data(void) {
  ...
  long_gps = long_gps_;
  alt_sl_gps.WW =
      alt_sl_gps_.WW / 10;  // SIRF provides altMSL in cm, UBX provides it in mm
...
#endif
  cog_gps.BB = (int16_t)(cog_gps_.WW / 1000);  // SIRF uses 2 byte COG, 10^-2
                                               // deg, UBX provides 4 bytes,
                                               // 10^-5 deg

  climb_gps.BB =
      -climb_gps_._.W0;  // SIRF uses 2 byte climb rate, UBX provides 4 bytes
